...
public class DiscountCodeManager {
    ...
    private void initDatabase() throws IOException {
        ...
        this.codes.clear();
        this.codes.addAll(this.config.getStringList("codes").stream().map(DiscountCode::fromString)
    ...
    @NotNull
    public CodeCreationResponse createDiscountCode(@NotNull CommandSender sender, @NotNull UUID owner,
            @NotNull String code, @NotNull CodeType codeType, @NotNull String rate, int maxUsage, double threshold,
            long expiredTime)
    {
        ...
        if (!QuickShop.getPermissionManager().hasPermission(sender,
        ...
        DiscountCode discountCode = new DiscountCode(owner, code, codeType, discountRate, maxUsage, threshold,
                expiredTime);

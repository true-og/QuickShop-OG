...
public class DiscountCommand implements CommandHandler<CommandSender> {
        ...
        if (parser.getArgs().isEmpty()) {
            quickshop
                    ...
                    .of(
                            sender,
                            "command-incorrect",
                            "/quickshop discount <install/uninstall/create/remove/list/config/query>")
                    .send();
            return;
        }
        ...
        switch (parser.getArgs().get(0)) {
            ...
            default -> quickshop
                    ...
                    .of(
                            sender,
                            "command-incorrect",
                            "/quickshop discount <install/uninstall/create/remove/config/query>")
                    .send();
        }
    }
    ...
    private void install(CommandSender sender, String[] passThroughArgs) {
        ...
        if (passThroughArgs.length < 1) {
            quickshop
                    .text()
                    .of(sender, "command-incorrect", "/quickshop discount install <code>")
                    .send();
            return;
        }
        ...
        quickshop
                .text()
                .of(sender, "addon.discount.discount-code-installed", code.getCode())
                .send();
    }
    ...
    private void create(CommandSender sender, String[] passThroughArgs) {
        ...
        if (codeType == null) {
            quickshop
                    .text()
                    .of(sender, "addon.discount.invalid-code-type", codeTypeStr)
                    .send();
            return;
        }
        int maxUsages = -1;
        ...
        if (passThroughArgs.length >= 6 && !"-1".equalsIgnoreCase(passThroughArgs[5])) {
            ...
            if (date == null) {
                quickshop
                        .text()
                        .of(sender, "not-a-valid-time", passThroughArgs[5])
                        .send();
                return;
            }
            expiredOn = date.getTime();
        }
        CodeCreationResponse response = main.getCodeManager()
                .createDiscountCode(
                        p, p.getUniqueId(), code, codeType, passThroughArgs[2], maxUsages, threshold, expiredOn);
        switch (response) {
            case PERMISSION_DENIED -> quickshop
                    .text()
                    .of(sender, "no-permission")
                    .send();
            case INVALID_RATE -> quickshop
                    .text()
                    .of(sender, "addon.discount.invalid-discount-rate")
                    .send();
            case REGEX_FAILURE -> quickshop
                    .text()
                    .of(sender, "addon.discount.invalid-discount-code-regex", DiscountCodeManager.NAME_REG_EXP)
                    .send();
            case INVALID_USAGE -> quickshop
                    .text()
                    .of(sender, "addon.discount.invalid-usage-restriction")
                    .send();
            case INVALID_THRESHOLD -> quickshop
                    .text()
                    .of(sender, "addon.discount.invalid-threshold-restriction")
                    .send();
            case INVALID_EXPIRE_TIME -> quickshop
                    .text()
                    .of(sender, "addon.discount.invalid-expire-time")
                    .send();
            case CODE_EXISTS -> quickshop
                    .text()
                    .of(sender, "addon.discount.discount-code-already-exists")
                    .send();
            case SUCCESS -> quickshop
                    .text()
                    .of(
                            sender,
                            "addon.discount.discount-code-created-successfully",
                            code,
                            CommonUtil.prettifyText(codeType.name()),
                            "/quickshop discount install " + code,
                            "/quickshop discount config " + code + " addshop")
                    .send();
        }
    ...
    private void remove(CommandSender sender, String[] passThroughArgs) {
        if (passThroughArgs.length < 1) {
            quickshop
                    .text()
                    .of(sender, "command-incorrect", "/quickshop discount remove <code>")
                    .send();
            return;
        }
        String codeStr = passThroughArgs[0];
        ...
        quickshop
                .text()
                .of(sender, "addon.discount.discount-code-removed", code.getCode())
                .send();
    }
    ...
    private void info(CommandSender sender, String[] passThroughArgs) {
        ...
        if (code == null) {
            quickshop
                    .text()
                    .of(sender, "addon.discount.discount-code-query-nothing")
                    .send();
            return;
        }
        String name = "Unknown";
        ...
        Component appliedTo = quickshop
                .text()
                .of(sender, "addon.discount.code-type." + code.getCodeType().name())
                .forLocale();
        String remainsUsage;
        ...
        quickshop
                ...
                .of(
                        sender,
                        "addon.discount.discount-code-details",
                        code.getCode(),
                        name,
                        appliedTo,
                        remainsUsage,
                        expiredOn,
                        code.getThreshold(),
                        code.getRate().format(sender, quickshop.text()))
                .send();
    }
    ...
    private void config(CommandSender sender, String[] passThroughArgs) {
        ...
        if (passThroughArgs.length < 2) {
            quickshop
                    ...
                    .of(
                            sender,
                            "command-incorrect",
                            "/quickshop discount config <code> <addshop/removeshop/scope> [args]")
                    .send();
            return;
        }
        ...
        if (code == null) {
            quickshop
                    .text()
                    .of(sender, "addon.discount.invalid-discount-code", passThroughArgs[0])
                    .send();
            return;
        }
        switch (passThroughArgs[1]) {
            case "addshop" -> {
                ...
                if (code.getShopScope().add(shop.getShopId())) {
                    quickshop
                            .text()
                            .of(sender, "addon.discount.discount-code-config-shop-added", shop.getShopId())
                            .send();
                    main.getCodeManager().saveDatabase();
                } else {
                    quickshop
                            .text()
                            .of(sender, "addon.discount.discount-code-config-shop-add-failure", shop.getShopId())
                            .send();
                }
            }
            case "removeshop" -> {
                ...
                if (code.getShopScope().remove(shop.getShopId())) {
                    quickshop
                            .text()
                            .of(sender, "addon.discount.discount-code-config-shop-removed", shop.getShopId())
                            .send();
                    main.getCodeManager().saveDatabase();
                } else {
                    quickshop
                            .text()
                            .of(sender, "addon.discount.discount-code-config-shop-remove-failure", shop.getShopId())
                            .send();
                }
            }
            case "scope" -> {
                if (passThroughArgs.length < 3) {
                    ...
                    quickshop
                            .text()
                            .of(sender, "command-incorrect", "/quickshop discount config <code> scope <scope>")
                            .send();
                    return;
                }
                if (!code.getOwner().equals(((Player) sender).getUniqueId())
                        && !quickshop.perm().hasPermission(sender, "quickshopaddon.discount.bypass")) {
                    quickshop.text().of(sender, "no-permission").send();
                    return;
                }
                ...
                try {
                    ...
                    quickshop
                            .text()
                            .of(sender, "addon.discount.invalid-code-type", newScope)
                            .send();
                }
            }
        }
    ...
    private void list(CommandSender sender, String[] passThroughArgs) {
        ...
        printer.printHeader();
        printer.printLine(
                quickshop.text().of(sender, "addon.discount.discount-code-list").forLocale());
        main.getCodeManager().getCodes().stream()
                .filter(code -> code.getOwner().equals(((Player) sender).getUniqueId()))
                .forEach(
                        code -> printer.printLine(Component.text(code.getCode()).color(NamedTextColor.AQUA)));
        printer.printFooter();
    }
    ...
    public @Nullable List<String> onTabComplete(
        ...
        if (cmdArg.length == 2) {
            return switch (cmdArg[0]) {
                case "install", "uninstall", "remove", "create", "config" -> List.of(
                        PlainTextComponentSerializer.plainText()
                                .serialize(quickshop
                                        .text()
                                        .of(sender, "addon.discount.tab-complete.discount.general.code")
                                        .forLocale()));
        ...
        if (cmdArg.length == 3) {
            return switch (cmdArg[0]) {
                case "create" -> Arrays.stream(CodeType.values())
                        .map(Enum::name)
                        .toList();
    ...
    private List<String> tabCompleteHint(@NotNull CommandSender sender, @NotNull String key) {
        String str = PlainTextComponentSerializer.plainText()
                .serialize(quickshop.text().of(sender, key).forLocale());

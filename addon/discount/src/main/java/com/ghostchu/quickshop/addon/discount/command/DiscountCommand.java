...
public class DiscountCommand implements CommandHandler<CommandSender> {
    ...
    public void onCommand(CommandSender sender, @NotNull String commandLabel, @NotNull CommandParser parser) {
        ...
        if (parser.getArgs().isEmpty()) {
            ...
            quickshop.text().of(sender, "command-incorrect",
                    "/quickshop discount <install/uninstall/create/remove/list/config/query>").send();
            return;
        ...
        switch (parser.getArgs().get(0)) {
            ...
            default -> quickshop.text().of(sender, "command-incorrect",
                    "/quickshop discount <install/uninstall/create/remove/config/query>").send();
    ...
    private void create(CommandSender sender, String[] passThroughArgs) {
        ...
        CodeCreationResponse response = main.getCodeManager().createDiscountCode(p, p.getUniqueId(), code, codeType,
                passThroughArgs[2], maxUsages, threshold, expiredOn);
        switch (response) {
            ...
            case REGEX_FAILURE -> quickshop.text()
                    .of(sender, "addon.discount.invalid-discount-code-regex", DiscountCodeManager.NAME_REG_EXP).send();
            ...
            case SUCCESS -> quickshop.text()
                    .of(sender, "addon.discount.discount-code-created-successfully", code,
                            CommonUtil.prettifyText(codeType.name()), "/quickshop discount install " + code,
    ...
    private void info(CommandSender sender, String[] passThroughArgs) {
        ...
        Component appliedTo = quickshop.text().of(sender, "addon.discount.code-type." + code.getCodeType().name())
                .forLocale();
        String remainsUsage;
        ...
        quickshop
                .text().of(sender, "addon.discount.discount-code-details", code.getCode(), name, appliedTo,
                        remainsUsage, expiredOn, code.getThreshold(), code.getRate().format(sender, quickshop.text()))
                .send();
    ...
    private void config(CommandSender sender, String[] passThroughArgs) {
        ...
        if (passThroughArgs.length < 2) {
            ...
            quickshop.text().of(sender, "command-incorrect",
                    "/quickshop discount config <code> <addshop/removeshop/scope> [args]").send();
            return;
        ...
        switch (passThroughArgs[1]) {
            ...
            case "addshop" -> {
                ...
                if (code.getShopScope().add(shop.getShopId())) {
                    ...
                    quickshop.text().of(sender, "addon.discount.discount-code-config-shop-added", shop.getShopId())
                            .send();
            ...
            case "removeshop" -> {
                ...
                if (code.getShopScope().remove(shop.getShopId())) {
                    ...
                    quickshop.text().of(sender, "addon.discount.discount-code-config-shop-removed", shop.getShopId())
                            .send();
                    main.getCodeManager().saveDatabase();
            ...
            case "scope" -> {
                ...
                if (passThroughArgs.length < 3) {
                    ...
                    quickshop.text().of(sender, "command-incorrect", "/quickshop discount config <code> scope <scope>")
                            .send();
                    return;
                ...
                try {
                    ...
                    if (!quickshop.perm().hasPermission(sender,
                            "quickshopaddon.discount.create." + type.name().toLowerCase()))
                    {
        ...
        if (cmdArg.length == 2) {
            ...
            return switch (cmdArg[0]) {
                ...
                case "install", "uninstall", "remove", "create", "config" ->
                    List.of(PlainTextComponentSerializer.plainText().serialize(quickshop.text()
                            .of(sender, "addon.discount.tab-complete.discount.general.code").forLocale()));

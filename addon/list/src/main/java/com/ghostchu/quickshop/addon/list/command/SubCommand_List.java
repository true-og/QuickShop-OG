    ...
    private void lookup(@NotNull Player sender, @NotNull UUID lookupUser) {
        ...
        for (Shop shop : shops) {
            ...
            Component shopNameComponent = LegacyComponentSerializer.legacySection().deserialize(shopName)
                    .append(Component.textOfChildren(Component.text(" (").append(Util.getItemStackName(shop.getItem()))
            ...
            Component component = quickshop.text()
                    .of(sender, "addon.list.entry", counter, shopNameComponent, location.getWorld().getName(),
                            location.getBlockX(), location.getBlockY(), location.getBlockZ(),
                            quickshop.getEconomy().format(shop.getPrice(), shop.getLocation().getWorld(),
                                    shop.getCurrency()),
                            shop.getShopStackingAmount(), Util.getItemStackName(shop.getItem()), shopTypeComponent)
                    .forLocale();
            component = component
                    .clickEvent(ClickEvent.runCommand("/quickshop silentpreview " + shop.getRuntimeRandomUniqueId()));

...
public class OpenInvCommand implements CommandHandler<Player> {
    ...
    public void onCommand(Player sender, @NotNull String commandLabel, @NotNull String[] cmdArg) {
        ...
        if (shop.getInventory() instanceof EnderChestWrapper) {
            ...
            sender.sendMessage(
                    ChatColor.translateAlternateColorCodes('&', plugin.getConfig().getString("messages.to-chest")));

        } else {

            shop.setInventory(
                    new EnderChestWrapper(shop.getOwner().getUniqueIdIfRealPlayer().orElse(CommonUtil.getNilUniqueId()),
                            plugin.getOpenInv(), plugin),
                    plugin.getManager());
            sender.sendMessage(
                    ChatColor.translateAlternateColorCodes('&', plugin.getConfig().getString("messages.to-echest")));
     ...
     *                     specified instance
     * @param commandLabel The command prefix (/quickshop = qs, /shop = shop)
     * @param cmdArg       The arguments (/quickshop create stone [TAB] will receive
     *                     stone)

...
public class QuickShopBukkit extends JavaPlugin {
                ...
                case 1 -> {
                    ...
                    this.platform = switch (AbstractSpigotPlatform.getNMSVersion()) {
                        ...
                        default -> {
                            getLogger()
                                    .warning(
                                            "This server running " + AbstractSpigotPlatform.getNMSVersion()
                                                    + " not supported by Hikari. (Try update? or Use Paper's fork to get cross-platform compatibility.)");
                            Bukkit.getPluginManager().disablePlugin(this);
    ...
    static class UnirestLibLoader {
        public UnirestLibLoader(QuickShopBukkit plugin) {
            ...
            Unirest.config()
                    ...
                    .setDefaultHeader(
                            "User-Agent",
                            "QuickShop/" + plugin.getFork() + "-"
                                    + plugin.getDescription().getVersion() + " Java/"
                                    + System.getProperty("java.version"));
            Unirest.config()
                    .verifySsl(PackageUtil.parsePackageProperly("verifySSL").asBoolean());
            if (PackageUtil.parsePackageProperly("proxyHost").isPresent()) {
                plugin.getLogger().info("Unirest proxy feature has been enabled.");
                Unirest.config()
                        .proxy(
                                PackageUtil.parsePackageProperly("proxyHost").asString("127.0.0.1"),
                                PackageUtil.parsePackageProperly("proxyPort").asInteger(1080));
            }
            if (PackageUtil.parsePackageProperly("proxyUsername").isPresent()) {
                ...
                Unirest.config()
                        .proxy(
                                PackageUtil.parsePackageProperly("proxyHost").asString("127.0.0.1"),
                                PackageUtil.parsePackageProperly("proxyPort").asInteger(1080),
                                PackageUtil.parsePackageProperly("proxyUsername")
                                        .asString(""),
                                PackageUtil.parsePackageProperly("proxyPassword")
                                        .asString(""));
            }
        }
    }
    ...
    static class AdventureLibLoader {
        public AdventureLibLoader(QuickShopBukkit plugin) {
            ...
            plugin.getLogger()
                    .info("Adventure Bukkit Platform loaded from: " + CommonUtil.getClassPath(BukkitAudiences.class));
            plugin.getLogger()
                    .info("Adventure Text Serializer (Legacy) loaded from: "
                            + CommonUtil.getClassPath(LegacyComponentSerializer.class));
            plugin.getLogger()
                    .info("Adventure Text Serializer (Gson) loaded from: "
                            + CommonUtil.getClassPath(GsonComponentSerializer.class));
            plugin.getLogger()
                    .info("Adventure Text Serializer (Json) loaded from: "
                            + CommonUtil.getClassPath(JSONComponentSerializer.class));
            plugin.getLogger()
                    .info("Adventure Text Serializer (BungeeChat) loaded from: "
                            + CommonUtil.getClassPath(BungeeComponentSerializer.class));
            plugin.getLogger()
                    .info("Adventure Text Serializer (ViaVersion Facet) loaded from: "
                            + CommonUtil.getClassPath(ViaFacet.class));
            plugin.getLogger()
                    .info("Adventure Text Serializer (ANSI) loaded from: "
                            + CommonUtil.getClassPath(ANSIComponentSerializer.class));
            plugin.getLogger()
                    .info("Adventure Text Serializer (Plain) loaded from: "
                            + CommonUtil.getClassPath(PlainTextComponentSerializer.class));
            plugin.getLogger()
                    .info("Adventure MiniMessage Lib loaded from: " + CommonUtil.getClassPath(MiniMessage.class));
        }
    }
}

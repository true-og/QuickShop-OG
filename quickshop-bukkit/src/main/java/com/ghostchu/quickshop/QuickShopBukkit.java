...
public class QuickShopBukkit extends JavaPlugin {
                ...
                case 1 -> {
                    ...
                    this.platform = switch (AbstractSpigotPlatform.getNMSVersion()) {
                        ...
                        default -> {
                            ...
                            getLogger().warning("This server running " + AbstractSpigotPlatform.getNMSVersion()
                                    + " not supported by Hikari. (Try update? or Use Paper's fork to get cross-platform compatibility.)");
                            Bukkit.getPluginManager().disablePlugin(this);
                            throw new IllegalStateException("This server running "
                                    + AbstractSpigotPlatform.getNMSVersion()
                                    + " not supported by Hikari. (Try update? or Use Paper's fork to get cross-platform compatibility.)");
    ...
    /**
     * Returns QS version, this method only exist on QuickShop forks If running
     * other QuickShop forks, result may not is "Reremake x.x.x" If running QS
     * official, Will throw exception.
     *
    ...
    static class UnirestLibLoader {
        ...
        public UnirestLibLoader(QuickShopBukkit plugin) {
            ...
            if (PackageUtil.parsePackageProperly("proxyHost").isPresent()) {
                ...
                Unirest.config().proxy(PackageUtil.parsePackageProperly("proxyHost").asString("127.0.0.1"),
            ...
            if (PackageUtil.parsePackageProperly("proxyUsername").isPresent()) {
                ...
                Unirest.config().proxy(PackageUtil.parsePackageProperly("proxyHost").asString("127.0.0.1"),
                        PackageUtil.parsePackageProperly("proxyPort").asInteger(1080),
                        PackageUtil.parsePackageProperly("proxyUsername").asString(""),
                        PackageUtil.parsePackageProperly("proxyPassword").asString(""));
    ...
    static class AdventureLibLoader {
        ...
        public AdventureLibLoader(QuickShopBukkit plugin) {
            ...
            plugin.getLogger().info("Adventure Text Serializer (Legacy) loaded from: "

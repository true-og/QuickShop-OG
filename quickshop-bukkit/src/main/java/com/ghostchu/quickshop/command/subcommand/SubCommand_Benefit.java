...
public class SubCommand_Benefit implements CommandHandler<Player> {
    ...
    public void onCommand(@NotNull Player sender, @NotNull String commandLabel, @NotNull CommandParser parser) {
        ...
        if (parser.getArgs().isEmpty()) {
            ...
            plugin.text().of(sender, "command-incorrect", "/quickshop benefit <add/remove/query> <player> <percentage>")
                    .send();
            return;
        ...
        switch (parser.getArgs().get(0)) {
            ...
            default -> plugin.text()
                    .of(sender, "command-incorrect", "/quickshop benefit <add/remove> <player> <percentage>").send();
    ...
    private void addBenefit(Player sender, Shop shop, @NotNull CommandParser parser) {
        ...
        if (parser.getArgs().size() < 3) {
            ...
            plugin.text().of(sender, "command-incorrect", "/quickshop benefit <add/remove> <player> <percentage>")
                    .send();
            return;
    ...
    private void removeBenefit(Player sender, Shop shop, @NotNull CommandParser parser) {
        ...
        plugin.getPlayerFinder().name2UuidFuture(player).whenComplete((uuid, throwable) -> {
            ...
            shop.setShopBenefit(benefit);
            plugin.text().of(sender, "benefit-removed", MsgUtil.formatPlayerProfile(new Profile(uuid, player), sender))
                    .send();
    ...
    private void queryBenefit(Player sender, Shop shop, @NotNull CommandParser parser) {
        ...
        Util.asyncThreadRun(() -> {
            ...
            for (Map.Entry<UUID, Double> entry : shop.getShopBenefit().getRegistry().entrySet()) {
                ...
                plugin.text().of(sender, "benefit-query-list", plugin.getPlayerFinder().uuid2Name(entry.getKey()),
                        entry.getKey(), v + "%").send();
            ...
            @NotNull CommandParser parser)
    {
        ...
        if (parser.getArgs().size() == 3) {
            ...
            return Collections.singletonList(LegacyComponentSerializer.legacySection()
                    .serialize(plugin.text().of(sender, "tabcomplete.percentage").forLocale()));

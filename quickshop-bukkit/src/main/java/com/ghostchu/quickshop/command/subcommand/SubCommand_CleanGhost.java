    ...
    public void onCommand(@NotNull CommandSender sender, @NotNull String commandLabel, @NotNull CommandParser parser) {
        ...
        updateExecutor.startHandle(plugin.getJavaPlugin(), (shop) -> {
            ...
            if (shop.getOwner() == null) {
                ...
                plugin.logEvent(
                        new ShopRemoveLog(QUserImpl.createFullFilled(CommonUtil.getNilUniqueId(), "SYSTEM", false),
                                "/quickshop cleanghost command", shop.saveToInfoStorage()));
                return;
            ...
            if (!Util.canBeShop(shop.getLocation().getBlock())) {
                ...
                (aVoid, throwable) -> plugin.text().of(sender, "cleanghost-deleted", deletionCounter.get()).send());

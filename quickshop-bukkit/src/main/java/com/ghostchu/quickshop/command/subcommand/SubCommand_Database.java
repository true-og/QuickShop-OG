...
public class SubCommand_Database implements CommandHandler<CommandSender> {
     ...
     * @param cmdArg       The arguments (/quickshop create stone will receive
     *                     stone)
     */
    @Override
    ...
    private void handleTrim(@NotNull CommandSender sender, @NotNull List<String> subParams) {
        ...
        databaseHelper.purgeIsolated().thenAccept(i -> plugin.text().of(sender, "database.trim-complete", i).send())
                .exceptionally(err ->
                {
    ...
    // DatabaseStatusHolder holder =
    // plugin.getDatabaseMaintenanceWatcher().getResult();
    // if (holder == null) {
    ...
    // case MAINTENANCE_REQUIRED -> plugin.text().of(sender,
    // "database.status-bad").forLocale();
    // };
    // ChatSheetPrinter printer = new ChatSheetPrinter(sender);
    // printer.printHeader();
    // printer.printLine(plugin.text().of(sender, "database.status",
    // statusComponent).forLocale());
    ...
    // printer.printLine(plugin.text().of(sender,
    // "database.suggestion.trim").forLocale());
    // }
    // printer.printFooter();
    // }
    ...
    private void purgeLogs(@NotNull CommandSender sender, @NotNull List<String> subParams) {
        ...
        try {
            ...
            databaseHelper.purgeIsolated()
                    .whenComplete((data, err) -> plugin.text().of(sender, "database.trim-complete", data).send());
    ...
    private void purgePlayersCache(CommandSender sender, @NotNull List<String> subParams) {
        ...
        Util.asyncThreadRun(() -> DataTables.PLAYERS.createDelete().build().executeAsync((lines) -> {

...
public class SubCommand_Help implements CommandHandler<CommandSender> {
    ...
    private void sendHelp(@NotNull CommandSender s, @NotNull String commandLabel) {
        ...
        commandPrintingLoop: for (CommandContainer container : plugin.getCommandManager().getRegisteredCommands()) {
            ...
            if (!container.isHidden()) {
                        ...
                        || (container.getDisabledSupplier() != null && container.getDisabledSupplier().get()))
                {
                    ...
                    if (plugin.perm().hasPermission(s, "quickshop.showdisabled")) {
                        ...
                        plugin.text().of(s, "command.format-disabled", commandLabel, container.getPrefix(),
                                container.getDisableText(s)).send();

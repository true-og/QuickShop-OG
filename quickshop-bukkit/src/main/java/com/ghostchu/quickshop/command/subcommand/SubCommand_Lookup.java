...
public class SubCommand_Lookup implements CommandHandler<Player> {
    ...
    public void onCommand(@NotNull Player sender, @NotNull String commandLabel, @NotNull CommandParser parser) {
        ...
        if (parser.getArgs().isEmpty()) {
            plugin.text()
                    .of(sender, "command-incorrect", "/quickshop lookup <create/remove/test> <name>")
                    .send();
            return;
        }
        ...
        switch (parser.getArgs().get(0).toLowerCase(Locale.ROOT)) {
            case "create" -> {
                ...
                switch (result) {
                    case SUCCESS -> plugin.text()
                            .of(sender, "lookup-item-created", itemRefName)
                            .send();
                    case REGEXP_FAILURE -> plugin.text()
                            .of(
                                    sender,
                                    "lookup-item-name-regex",
                                    ItemMarker.getNameRegExp(),
                                    parser.getArgs().get(1))
                            .send();
                    case NAME_CONFLICT -> plugin.text()
                            .of(sender, "lookup-item-exists", itemRefName)
                            .send();
                    default -> plugin.text()
                            .of(sender, "internal-error", itemRefName)
                            .send();
                }
            }
            case "remove" -> {
                ...
                switch (result) {
                    case SUCCESS -> plugin.text()
                            .of(sender, "lookup-item-removed", itemRefName)
                            .send();
                    case NOT_EXISTS -> plugin.text()
                            .of(sender, "lookup-item-not-found", itemRefName)
                            .send();
                    default -> plugin.text()
                            .of(sender, "internal-error", itemRefName)
                            .send();

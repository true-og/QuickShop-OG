...
public class SubCommand_Name implements CommandHandler<Player> {
    ...
    public void onCommand(@NotNull Player sender, @NotNull String commandLabel, @NotNull CommandParser parser) {
        ...
        if (fee > 0) {
            ...
            if (!plugin.perm().hasPermission(sender, "quickshop.bypass.namefee")) {
                ...
                transaction = SimpleEconomyTransaction.builder().world(shop.getLocation().getWorld())
                        .from(sender.getUniqueId()).to(shop.getTaxAccount()).currency(plugin.getCurrency())
                        .taxAccount(shop.getTaxAccount()).taxModifier(0.0d).core(plugin.getEconomy()).amount(fee)
                        .build();
                if (!transaction.checkBalance()) {

                    plugin.text().of(sender, "you-cant-afford-shop-naming",
                            plugin.getShopManager().format(fee, shop.getLocation().getWorld(), plugin.getCurrency()))
                            .send();
                    return;
            ...
            @NotNull CommandParser parser)
    {
        ...
        return parser.getArgs().size() == 1 ? Collections.singletonList(LegacyComponentSerializer.legacySection()
                .serialize(plugin.text().of(sender, "tabcomplete.name").forLocale())) : Collections.emptyList();

...
public class SubCommand_Price implements CommandHandler<Player> {
    ...
    public void onCommand(@NotNull Player sender, @NotNull String commandLabel, @NotNull CommandParser parser) {
        ...
        switch (checkResult.getStatus()) {
            ...
            case PRICE_RESTRICTED -> {
                ...
                plugin.text().of(sender, "restricted-prices", Util.getItemStackName(shop.getItem()),
                        Component.text(checkResult.getMin()), Component.text(checkResult.getMax())).send();
                return;
            ...
            case REACHED_PRICE_MIN_LIMIT -> {
                ...
                plugin.text().of(sender, "price-too-cheap",
                        (format) ? MsgUtil.decimalFormat(checkResult.getMin()) : Double.toString(checkResult.getMin()))
                        .send();
                return;
            ...
            case REACHED_PRICE_MAX_LIMIT -> {
                ...
                plugin.text().of(sender, "price-too-high",
                        (format) ? MsgUtil.decimalFormat(checkResult.getMax()) : Double.toString(checkResult.getMax()))
                        .send();
                return;
        ...
        if (fee > 0) {
            ...
            SimpleEconomyTransaction transaction = SimpleEconomyTransaction.builder().core(plugin.getEconomy())
                    .from(sender.getUniqueId()).amount(fee).world(Objects.requireNonNull(shop.getLocation().getWorld()))
                    .currency(plugin.getCurrency()).build();
            if (!transaction.checkBalance()) {

                plugin.text().of(sender, "you-cant-afford-to-change-price", plugin.getShopManager().format(fee, shop))
                        .send();
                return;
        ...
        plugin.text().of(sender, "fee-charged-for-price-change", plugin.getShopManager().format(fee, shop)).send();
        // Update the shop
        shop.setPrice(price);
        shop.setSignText(plugin.text().findRelativeLanguages(sender));
        plugin.text().of(sender, "price-is-now", plugin.getShopManager().format(shop.getPrice(), shop)).send();

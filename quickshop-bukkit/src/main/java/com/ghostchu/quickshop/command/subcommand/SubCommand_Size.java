    ...
    public void onCommand(@NotNull Player sender, @NotNull String commandLabel, @NotNull CommandParser parser) {
        ...
        if (shop != null) {
                    ...
                    || plugin.perm().hasPermission(sender, "quickshop.other.amount"))
            {
                ...
                if (amount > Util.getItemMaxStackSize(shop.getItem().getType())
                        && !plugin.getConfig().getBoolean("shop.disable-max-size-check-for-size-command", false))
                {
                ...
                PriceLimiterCheckResult checkResult = limiter.check(sender, pendingItemStack, shop.getCurrency(),
                        shop.getPrice());
                ...
                plugin.text().of(sender, "command.bulk-size-now", shop.getItem().getAmount(),

...
public class SubCommand_Transfer implements CommandHandler<Player> {
    ...
    private final Cache<UUID, PendingTransferTask> taskCache = CacheBuilder.newBuilder()
    ...
    public void onCommand(@NotNull Player sender, @NotNull String commandLabel, @NotNull CommandParser parser) {
        ...
        if (parser.getArgs().size() == 2) {
            ...
            Util.asyncThreadRun(() -> {
                ...
                QUser fromQUser = QUserImpl.createSync(QuickShop.getInstance().getPlayerFinder(),
                        parser.getArgs().get(0));
                QUser targetQUser = QUserImpl.createSync(QuickShop.getInstance().getPlayerFinder(),
                ...
                Util.mainThreadRun(() -> {
                    ...
                    task.commit(false);
                    plugin.text().of(sender, "command.transfer-success-other", shopList.size(), parser.getArgs().get(0),
                            parser.getArgs().get(1)).send();

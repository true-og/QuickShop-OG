...
public class MetricQuery {
    ...
    public long queryServerPurchaseCount() {
        ...
        try (SQLQuery query = databaseHelper.getManager().createQuery().withPreparedSQL(sql)
                .setParams(Collections.emptyList()).execute())
        {
    ...
    public List<ShopTransactionRecord> queryTransactions(@NotNull Date startTime, long limit, boolean descending) {
        ...
        try (SQLQuery query = databaseHelper.getManager().createQuery()
                .inTable(databaseHelper.getPrefix() + "log_transaction").addTimeCondition("time", startTime, null)
                .selectColumns().setLimit(1000).orderBy("id", !descending).build().execute())
        {
            ...
            while (set.next()) {
                ...
                ShopTransactionRecord record = new ShopTransactionRecord(set.getDate("time"),
                        UUID.fromString(set.getString("from")), UUID.fromString(set.getString("to")),
                        set.getString("currency"), set.getDouble("amount"),
                        UUID.fromString(set.getString("tax_currency")), set.getDouble("tax_amount"),
                        set.getString("error"));
                list.add(record);
    ...
    public List<ShopMetricRecord> queryServerPurchaseRecords(@NotNull Date startTime, int limit, boolean descending) {
        ...
        try (SQLQuery query = databaseHelper.getManager().createQuery()
                .inTable(databaseHelper.getPrefix() + "log_purchase").addTimeCondition("time", startTime, null)
                .selectColumns().setLimit(limit).orderBy("id", !descending).build().execute())
        {
            ...
            while (set.next()) {
                ...
                // "time", "shop", "data", "buyer", "type", "amount", "money", "tax"
                ShopMetricRecord record = ShopMetricRecord.builder().time(set.getDate("time").getTime())
                        .shopId(set.getLong("shop")).type(ShopOperationEnum.valueOf(set.getString("type")))
                        .total(set.getDouble("money")).tax(set.getDouble("tax")).amount(set.getInt("amount"))
                        .player(QUserImpl.createSync(plugin.getPlayerFinder(), set.getString("buyer"))).build();
                list.add(record);

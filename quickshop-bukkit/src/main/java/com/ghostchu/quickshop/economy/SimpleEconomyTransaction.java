...
/**
 * Transaction A secure way to transfer the money between players. Support
 * rollback :)
 */
@ToString
public class SimpleEconomyTransaction implements EconomyTransaction {
     ...
     */
    @Builder
    public SimpleEconomyTransaction(@Nullable Object from, @Nullable Object to, double amount, double taxModifier,
            @Nullable Object taxAccount, EconomyCore core, Boolean allowLoan, @NotNull World world,
            @Nullable String currency, boolean neverFail, @Nullable Benefit benefit)
    {
        ...
        this.allowLoan = Objects.requireNonNullElseGet(allowLoan,
                () -> plugin.getConfig().getBoolean("shop.allow-economy-loan", false));
        this.world = world;
    ...
    public interface SimpleTransactionCallback extends TransactionCallback {
        ...
        default void onFailed(@NotNull SimpleEconomyTransaction economyTransaction) {
            ...
            Log.transaction(Level.WARNING, "Transaction failed: " + economyTransaction.getLastError()
        ...
        default void onTaxFailed(@NotNull SimpleEconomyTransaction economyTransaction) {
            ...
            Log.transaction(Level.WARNING, "Tax Transaction failed: " + economyTransaction.getLastError()
                    + ", transaction: " + economyTransaction);
    ...
    public boolean commit(@NotNull TransactionCallback callback) {
        ...
        if (benefit.isEmpty()) {
            ...
            if (to != null
                    && !this.executeOperation(new DepositEconomyOperation(to, amountAfterTax, world, currency, core)))
            {

                this.lastError = "Failed to deposit " + amountAfterTax + " to player " + to + " account. LastError: "
                        + core.getLastError();
                callback.onFailed(this);
                return false;
            ...
            for (Map.Entry<UUID, Double> entry : benefit.getRegistry().entrySet()) {
                ...
                Log.transaction(
                        "Benefit for " + entry.getKey() + ", value: " + entry.getValue() + ". Payout = " + payout);
                if (!this.executeOperation(new DepositEconomyOperation(entry.getKey(),
                        amountAfterTax * entry.getValue(), world, currency, core)))
                {

                    this.lastError = "Failed to deposit " + amountAfterTax * entry.getValue() + " to player " + to
                            + " account. LastError: " + core.getLastError();
                    callback.onFailed(this);
                    return false;
            ...
            if (ownerCanGet > 0) {
                ...
                if (to != null
                        && !this.executeOperation(new DepositEconomyOperation(to, ownerCanGet, world, currency, core)))
                {

                    this.lastError = "Failed to deposit " + ownerCanGet + " to player " + to + " account. LastError: "
                            + core.getLastError();
                    callback.onFailed(this);
                    return false;
        ...
        if (tax > 0 && taxer != null
                && !this.executeOperation(new DepositEconomyOperation(taxer, tax, world, currency, core)))
        {
                ...
                try {
                    ...
                    if (continueWhenFailed) {
                        ...
                        plugin.logger().warn(
                                ...
                                core.getLastError(), operation, this);
    ...
    private boolean executeOperation(@NotNull Operation operation) {
        ...
        try {
            ...
            plugin.logger().warn("Failed to execute operation: {}; Operation: {}; Transaction: {}", core.getLastError(),
                    operation, this);

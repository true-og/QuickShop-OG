...
public class InternalListener extends AbstractQSListener {
    ...
    private final Cache<Shop, SpaceCache> countUpdateCache = CacheBuilder.newBuilder().weakKeys()
            .expireAfterAccess(5, TimeUnit.MINUTES).maximumSize(100).build();
    ...
    public void shopCreate(ShopCreateEvent event) {
        ...
        if (loggingAction) {
            ...
            plugin.logEvent(new ShopCreationLog(event.getCreator(), event.getShop().saveToInfoStorage(),
    ...
    public void shopDelete(ShopDeleteEvent event) {
        ...
        if (loggingAction) {
            ...
            plugin.logEvent(new ShopRemoveLog(QUserImpl.createFullFilled(CommonUtil.getNilUniqueId(), "SYSTEM", false),
                    "Shop removed", event.getShop().saveToInfoStorage()));
    ...
    public void shopInventoryCalc(ShopInventoryCalculateEvent event) {
        ...
        plugin.getDatabaseHelper()
                ...
                .exceptionally(err ->
                {
                    ...
                    Log.debug("Error updating external inventory profile cache for shop " + event.getShop().getShopId()
                            + ": " + err.getMessage());
                    return 0;
    ...
    public void shopPrePurchase(ShopPurchaseEvent event) {
        ...
        if (loggingBalance) {
            ...
            plugin.logEvent(new PlayerEconomyPreCheckLog(true, event.getPurchaser(), plugin.getEconomy().getBalance(
                    event.getPurchaser(), event.getShop().getLocation().getWorld(), event.getShop().getCurrency())));
            plugin.logEvent(new PlayerEconomyPreCheckLog(true, event.getShop().getOwner(),
                    plugin.getEconomy().getBalance(event.getShop().getOwner(), event.getShop().getLocation().getWorld(),
    ...
    public void shopPriceChanges(ShopPriceChangeEvent event) {
        ...
        if (loggingAction) {
            ...
            plugin.logEvent(new ShopPriceChangedLog(event.getShop().saveToInfoStorage(), event.getOldPrice(),
                    event.getNewPrice()));
    ...
    public void shopPurchase(ShopSuccessPurchaseEvent event) {
        ...
        if (loggingAction) {
            ...
            plugin.logEvent(new ShopPurchaseLog(event.getShop().saveToInfoStorage(), event.getShop().getShopType(),
                    event.getPurchaser(),
                    LegacyComponentSerializer.legacySection()
                            .serialize(Util.getItemStackName(event.getShop().getItem())),
                    Util.serialize(event.getShop().getItem()), event.getAmount(), event.getBalance(), event.getTax()));

        }

        if (loggingBalance) {

            plugin.logEvent(new PlayerEconomyPreCheckLog(false, event.getPurchaser(), plugin.getEconomy().getBalance(
                    event.getPurchaser(), event.getShop().getLocation().getWorld(), event.getShop().getCurrency())));
            plugin.logEvent(new PlayerEconomyPreCheckLog(false, event.getShop().getOwner(),
                    plugin.getEconomy().getBalance(event.getShop().getOwner(), event.getShop().getLocation().getWorld(),
                            event.getShop().getCurrency())));

...
public class PlayerListener extends AbstractQSListener {
    ...
    public void onAdventureClick(PlayerAnimationEvent event) {
        ...
        if (focused != null) {
            ...
            PlayerInteractEvent interactEvent = new PlayerInteractEvent(event.getPlayer(),
                    focused.getType() == Material.AIR ? Action.LEFT_CLICK_AIR : Action.LEFT_CLICK_BLOCK,
                    event.getPlayer().getInventory().getItemInMainHand(), focused,
        ...
        switch (plugin.getInteractionController().getBehavior(interaction)) {
            ...
            case TRADE_INTERACTION -> {
                ...
                if (shopSearched.getKey() == null) {
                    ...
                    if (e.getItem() != null && createShop(e.getPlayer(), e.getClickedBlock(), e.getBlockFace(),
                            e.getHand(), e.getItem()))
                    {
    ...
    public boolean createShop(@NotNull Player player, @Nullable Block block, @NotNull BlockFace blockFace,
            @NotNull EquipmentSlot hand, @NotNull ItemStack item)
    {
        ...
        if (plugin.getShopItemBlackList().isBlacklisted(stack)
                && !plugin.perm().hasPermission(player, "quickshop.bypass." + stack.getType().name()))
        {
        ...
        plugin.text()
                .of(player, "how-much-to-trade-for", Util.getItemStackName(stack),
    ...
    public boolean sellToShop(@NotNull Player p, @Nullable Shop shop, boolean direct, boolean all) {
                ...
                || plugin.perm().hasPermission(p, "quickshop.other.use"))
        {
            ...
            if (!direct) {
                ...
                plugin.getShopManager().actionBuying(p, new BukkitInventoryWrapper(p.getInventory()), eco, info, shop,
                        arg);
                ...
                || plugin.perm().hasPermission(p, "quickshop.other.use"))
        {
            ...
            if (!direct) {
                ...
                if (shop.isStackingShop()) {
                    ...
                    plugin.text().of(p, "how-many-buy-stack", shop.getItem().getAmount(), itemAmount, tradeAllWord)
                            .send();
                ...
                plugin.getShopManager().actionSelling(p, new BukkitInventoryWrapper(p.getInventory()), eco, info, shop,
                        arg);
    ...
    private int getPlayerCanSell(@NotNull Shop shop, double ownerBalance, double price,
            @NotNull InventoryWrapper playerInventory)
    {

        boolean isContainerCountingNeeded = shop.isUnlimited();
        if (shop.isFreeShop()) {

            return isContainerCountingNeeded ? Util.countItems(playerInventory, shop)
    ...
    private int buyingShopAllCalc(@NotNull AbstractEconomy eco, @NotNull Shop shop, @NotNull Player p) {
        ...
        if (amount < 1) { // typed 'all' but the auto set amount is 0
            ...
            if (shopHaveSpaces == 0) {
                ...
                plugin.text().of(p, "shop-has-no-space", shopHaveSpaces, Util.getItemStackName(shop.getItem())).send();
                return 0;
    ...
    private int getPlayerCanBuy(@NotNull Shop shop, double traderBalance, double price,
            @NotNull InventoryWrapper playerInventory)
    {
        ...
        if (shop.isFreeShop()) { // Free shop
            ...
            return isContainerCountingNeeded ? Util.countSpace(playerInventory, shop)
    ...
    public void onJoin(PlayerLocaleChangeEvent e) {
        ...
        plugin.getDatabaseHelper()
                .updatePlayerProfile(e.getPlayer().getUniqueId(), e.getLocale(), e.getPlayer().getName())
        ...
        if (plugin.perm().hasPermission(e.getPlayer(), "quickshop.alert")) {
                    ...
                    || PackageUtil.parsePackageProperly("april-rickandroll").asBoolean())
            {
                ...
                Bukkit.getScheduler().runTaskLater(plugin.getJavaPlugin(),
                        (() -> plugin.text().of(e.getPlayer(), "april-rick-and-roll-easter-egg").send()), 80L);
    ...
    public void onPlayerQuit(PlayerQuitEvent e) {
        ...
        plugin.getDatabaseHelper()
                .updatePlayerProfile(e.getPlayer().getUniqueId(), e.getPlayer().getLocale(), e.getPlayer().getName())
                .exceptionally(throwable ->
                {

...
public class ShopProtectionListener extends AbstractProtectionListener {
    ...
    private final NamespacedKey hopperKey = new NamespacedKey(QuickShop.getInstance().getJavaPlugin(),
            "hopper-persistent-data");
            ...
            if (shop != null) {
                ...
                if (plugin.getConfig().getBoolean("protect.explode")) {
                    ...
                    plugin.logEvent(new ShopRemoveLog(
                            ...
                            "BlockBreak(explode)", shop.saveToInfoStorage()));
    ...
    public void onInventoryMove(InventoryMoveItemEvent event) {
        ...
        if (this.hopperOwnerExclude) {
            ...
            if (event.getDestination().getHolder() instanceof Hopper hopper) {
                ...
                HopperPersistentData hopperPersistentData = hopper.getPersistentDataContainer().get(hopperKey,
                        HopperPersistentDataType.INSTANCE);
                if (hopperPersistentData != null) {

                    if (shop.playerAuthorize(hopperPersistentData.getPlayer(),
                            BuiltInShopPermission.ACCESS_INVENTORY))
                    {
    ...
    public void onPlaceHopper(BlockPlaceEvent e) {
        ...
        if (e.getBlockPlaced().getState() instanceof Hopper hopper) {
            ...
            hopper.getPersistentDataContainer().set(hopperKey, HopperPersistentDataType.INSTANCE,

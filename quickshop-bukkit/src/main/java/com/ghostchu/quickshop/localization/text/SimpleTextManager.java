...
public class SimpleTextManager implements TextManager, Reloadable, SubPasteItem {
    ...
    private final Cache<String, String> languagesCache = CacheBuilder.newBuilder()
            .expireAfterAccess(30, TimeUnit.MINUTES).recordStats().build();
    ...
    public SimpleTextManager(@NotNull QuickShop plugin) {
        ...
        this.crowdinHost = PackageUtil.parsePackageProperly("crowdinHost")
                .asString("https://crowdinota.hikari.r2.quickshop-powered.top");
        if (PackageUtil.parsePackageProperly("enableCrowdinOTA").asBoolean(true)) {
            ...
            try {
                ...
                this.crowdinOTA = new CrowdinOTA(crowdinHost, new File(Util.getCacheFolder(), "crowdin-ota"),
    ...
    public void load() {
        ...
        // TODO: This will break the message processing system in-game until loading
        // finished, need to fix it.
        this.reset();
        initTagResolvers();
            ...
            if (crowdinOTA != null) {
                ...
                if (fileInstance != null) {
                    ...
                    for (String crowdinCode : fileInstance.getAvailableLocales()) {
                        ...
                        String mcCode = crowdinOTA.mapLanguageCode(crowdinCode, LOCALE_MAPPING_SYNTAX)
    ...
    public @NotNull Text of(@NotNull String path, @Nullable Object... args) {
        ...
        return new Text(this, (CommandSender) null, languageFilesManager.getDistributions(), path, tagResolvers,
                convert(args));
    ...
    public @NotNull TextList ofList(@NotNull String path, Object... args) {
        ...
        return new TextList(this, (CommandSender) null, languageFilesManager.getDistributions(), path, tagResolvers,
                convert(args));
    ...
    @Override
    public com.ghostchu.quickshop.api.localization.text.@NotNull TextList ofList(@Nullable QUser sender,
            @NotNull String path, @Nullable Object... args)
    {
    ...
    public static class TextList implements com.ghostchu.quickshop.api.localization.text.TextList {
        ...
        private TextList(SimpleTextManager manager, CommandSender sender, Map<String, FileConfiguration> mapping,
                String path, TagResolver[] tagResolvers, Component... args)
        {
        ...
        private TextList(SimpleTextManager manager, UUID sender, Map<String, FileConfiguration> mapping, String path,
                TagResolver[] tagResolvers, Component... args)
        {
        ...
        public TextList(SimpleTextManager manager, QUser sender, Map<String, FileConfiguration> mapping, String path,
                TagResolver[] tagResolvers, Component... args)
        {
        ...
        public List<Component> forLocale(@NotNull String locale) {
            ...
            if (index == null) {
                ...
                Log.debug("Fallback " + locale
                        + " to default game-language locale caused by QuickShop doesn't support this locale");
                ...
                List<Component> components = str.stream()
                        .map(s -> manager.plugin.getPlatform().miniMessage().deserialize(s, tagResolvers)).toList();
            ...
            for (PostProcessor postProcessor : this.manager.postProcessors) {
                ...
                for (Component s : text) {
                    ...
                    try {
                        ...
                        Log.debug("Failed to post processing text: " + s + " caused by " + th.getMessage()
                                + " handler: " + postProcessor.getClass().getName());
    ...
    public static class Text implements com.ghostchu.quickshop.api.localization.text.Text {
        ...
        private Text(SimpleTextManager manager, CommandSender sender, Map<String, FileConfiguration> mapping,
                String path, TagResolver[] tagResolvers, Component... args)
        {
        ...
        private Text(SimpleTextManager manager, UUID sender, Map<String, FileConfiguration> mapping, String path,
                TagResolver[] tagResolvers, Component... args)
        {
        ...
        public Text(SimpleTextManager manager, QUser sender, Map<String, FileConfiguration> mapping, String path,
                TagResolver[] tagResolvers, Component... args)
        {
        ...
        private Component postProcess(@NotNull Component text) {
            ...
            for (PostProcessor postProcessor : this.manager.postProcessors) {
                ...
                try {
                    ...
                    Log.debug("Error occurred while processing text: "
                            + PlainTextComponentSerializer.plainText().serialize(text) + " caused by" + e.getMessage()
                            + ", handler: " + postProcessor.getClass().getName());

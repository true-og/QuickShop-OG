...
public class PAPICache implements Reloadable {
    ...
    private void init() {
        ...
        this.performCaches = CacheBuilder.newBuilder()
                .expireAfterWrite(expiredTime, java.util.concurrent.TimeUnit.MILLISECONDS).recordStats().build();

    }

    @NotNull
    public Optional<String> getCached(@NotNull UUID player, @NotNull String args,
            @NotNull BiFunction<UUID, String, String> loader)
    {

        try (PerfMonitor ignored = new PerfMonitor("PlaceHolder API Handling")) {

            return performCaches.get(compileUniqueKey(player, args),
                    () -> Optional.ofNullable(loader.apply(player, args)));
    ...
    private long getShopsInWorld(@NotNull String world, boolean loadedOnly) {
        ...
        return plugin.getShopManager().getAllShops().stream().filter(shop -> shop.getLocation().getWorld() != null)
                .filter(shop -> shop.getLocation().getWorld().getName().equals(world))
                .filter(shop -> !loadedOnly || shop.isLoaded()).count();
    ...
    private long getPlayerShopsInventoryUnavailable(@NotNull UUID uuid) {
        ...
        return plugin.getShopManager().getAllShops(uuid).stream().filter(Shop::inventoryAvailable).count();

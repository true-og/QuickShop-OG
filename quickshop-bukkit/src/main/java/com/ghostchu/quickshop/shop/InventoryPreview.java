...
public class InventoryPreview implements Listener {
    ...
    private static final NamespacedKey NAMESPACED_KEY = new NamespacedKey(QuickShop.getInstance().getJavaPlugin(),
            "preview-item");
    ...
    public void show(Player player) {
        ...
        if (inventory == null) {
            ...
            final int size = 9;
            inventory = Bukkit.createInventory(new QuickShopPreviewGUIHolder(), size, LegacyComponentSerializer
                    .legacySection().serialize(plugin.text().of(player, "menu.preview").forLocale()));

    ...
    private void run() {
        ...
        for (Shop shop : plugin.getShopManager().getAllShops()) {
            ...
            try {
                ...
                OfflinePlayer player = shop.getOwner().getUniqueIdIfRealPlayer().map(Bukkit::getOfflinePlayer)
                        .orElse(null);
                if (player == null) {
                ...
                long noOfDaysBetween = ChronoUnit.DAYS.between(CommonUtil.getDateTimeFromTimestamp(lastPlayed),
        ...
        purgeExecutor.startHandle(plugin.getJavaPlugin(), (shop) -> plugin.getShopManager().deleteShop(shop))
                .whenComplete((a, b) ->
                {
                    ...
                    long usedTime = purgeExecutor.getStartTime().until(Instant.now(),
                            java.time.temporal.ChronoUnit.MILLIS);
                    plugin.logger().info("[Shop Purger] Total shop {} has been purged, used {}ms",
                            pendingRemovalShops.size(), usedTime);

...
public class SimplePriceLimiter implements Reloadable, PriceLimiter, SubPasteItem {
    ...
    private RuleSet readRule(@NotNull String ruleName, @Nullable ConfigurationSection section) {
        ...
        for (String currencyStr1 : section.getStringList("currency")) {
            ...
            try {
                ...
                plugin.logger().warn("Failed to read rule {}'s a Currency option, invalid pattern {}! Skipping...",
                        ruleName, currencyStr1);
    ...
    @Override
    @NotNull
    public PriceLimiterCheckResult check(@NotNull CommandSender sender, @NotNull ItemStack itemStack,
            @Nullable String currency, double price)
    {
        ...
        if (wholeNumberOnly) {
            ...
            try {
                ...
                return new SimplePriceLimiterCheckResult(PriceLimiterStatus.NOT_A_WHOLE_NUMBER, undefinedMin,
                        undefinedMax);
    ...
    public @NotNull String genBody() {
        ...
        for (Map.Entry<String, RuleSet> entry : this.rules.entrySet()) {
            ...
            rules.insert(entry.getKey(), rule.getBypassPermission(), rule.getItems().size(), currencies,
    ...
    static class RuleSet {
        ...
        public RuleSet(List<Function<ItemStack, Boolean>> items, String bypassPermission, List<Pattern> currency,

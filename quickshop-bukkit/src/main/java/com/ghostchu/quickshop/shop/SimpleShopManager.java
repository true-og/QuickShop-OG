...
public class SimpleShopManager implements ShopManager, Reloadable {
    ...
    private final Cache<UUID, Shop> shopRuntimeUUIDCaching = CacheBuilder.newBuilder()
            .expireAfterAccess(10, TimeUnit.MINUTES).maximumSize(50).weakValues().initialCapacity(50).build();
    ...
    @Override
    public void actionBuying(@NotNull Player buyer, @NotNull InventoryWrapper buyerInventory,
            @NotNull AbstractEconomy eco, @NotNull Info info, @NotNull Shop shop, int amount)
    {

        QUser buyerQUser = QUserImpl.createFullFilled(buyer);
        if (!plugin.perm().hasPermission(buyer, "quickshop.other.use")
                && !shop.playerAuthorize(buyer.getUniqueId(), BuiltInShopPermission.PURCHASE))
        {
        ...
        if (space < amount) {
            ...
            plugin.text().of(buyer, "shop-has-no-space", Component.text(space), Util.getItemStackName(shop.getItem()))
                    .send();
            return;
        ...
        if (amount > count) {
            ...
            plugin.text().of(buyer, "you-dont-have-that-many-items", Component.text(count),
        ...
        SimpleEconomyTransaction.SimpleEconomyTransactionBuilder builder = SimpleEconomyTransaction.builder().core(eco)
                .amount(total).taxModifier(taxModifier).taxAccount(taxAccount).currency(shop.getCurrency())
                .world(shop.getLocation().getWorld()).to(buyer);
        ...
        if (!transaction.checkBalance()) {
            ...
            plugin.text()
                    .of(buyer, "the-owner-cant-afford-to-buy-from-you",
                            format(total, shop.getLocation().getWorld(), shop.getCurrency()),
                            format(eco.getBalance(shop.getOwner(), shop.getLocation().getWorld(), shop.getCurrency()),
        ...
        if (!transaction.failSafeCommit()) {
            ...
            plugin.logger().error(
                    "Tips: If you see any economy plugin name appears above, please don't ask QuickShop support. Contact with developer of economy plugin. QuickShop didn't process the transaction, we only receive the transaction result from your economy plugin.");
            return;
    ...
    private void notifySold(@NotNull QUser buyerQUser, @NotNull Shop shop, int amount, int space) {
        ...
        Util.asyncThreadRun(() -> {
            ...
            Component notify = plugin.text()
                    .of("player-sold-to-your-store", buyerQUser.getDisplay(), amount,
                            Util.getItemStackName(shop.getItem()))
                    .forLocale(langCode).hoverEvent(plugin.getPlatform().getItemStackHoverEvent(shop.getItem()));
            sendList.add(notify);
            if (space == amount) {
                ...
                if (shop.getShopName() == null) {
                    ...
                    spaceWarn = plugin.text()
                            .of("shop-out-of-space", shop.getLocation().getBlockX(), shop.getLocation().getBlockY(),
                    ...
                    spaceWarn = plugin.text()
                            .of("shop-out-of-space-name", shop.getShopName(), Util.getItemStackName(shop.getItem()))
                            .forLocale(langCode)
                            .hoverEvent(plugin.getPlatform().getItemStackHoverEvent(shop.getItem()));
    ...
    public boolean shopIsNotValid(@Nullable QUser qUser, @NotNull Info info, @NotNull Shop shop) {
        ...
        if (plugin.getEconomy() == null) {
            ...
            MsgUtil.sendDirectMessage(qUser,
                    Component.text("Error: Economy system not loaded, type /quickshop main command to get details.")
    ...
    @Override
    public void actionSelling(@NotNull Player seller, @NotNull InventoryWrapper sellerInventory,
            @NotNull AbstractEconomy eco, @NotNull Info info, @NotNull Shop shop, int amount)
    {
        ...
        if (!plugin.perm().hasPermission(seller, "quickshop.other.use")
                && !shop.playerAuthorize(seller.getUniqueId(), BuiltInShopPermission.PURCHASE))
        {
        ...
        if (stock < amount) {
            ...
            plugin.text().of(seller, "shop-stock-too-low", Component.text(stock), Util.getItemStackName(shop.getItem()))
                    .send();
            return;
        ...
        SimpleEconomyTransaction.SimpleEconomyTransactionBuilder builder = SimpleEconomyTransaction.builder().core(eco)
                .from(seller).amount(total).taxModifier(taxModifier).taxAccount(taxAccount)
                .benefit(shop.getShopBenefit()).world(shop.getLocation().getWorld()).currency(shop.getCurrency());
        if (shop.isUnlimited() && plugin.getConfig().getBoolean("tax-free-for-unlimited-shop", false)) {
        ...
        if (!transaction.checkBalance()) {
            ...
            plugin.text()
                    .of(seller, "you-cant-afford-to-buy",
                            format(total, shop.getLocation().getWorld(), shop.getCurrency()),
                            format(eco.getBalance(seller, shop.getLocation().getWorld(), shop.getCurrency()),
                                    shop.getLocation().getWorld(), shop.getCurrency()))
                    .send();
            return;
        ...
        new ShopSuccessPurchaseEvent(shop, sellerQUser, sellerInventory, amount, total, transaction.getTax())
                .callEvent();
    ...
    private void addShopToLookupTable(@NotNull Shop shop) {
        ...
        Map<ShopChunk, Map<Location, Shop>> inWorld = this.getShops().computeIfAbsent(world,
                k -> new MapMaker().initialCapacity(3).makeMap());
        ...
        Map<Location, Shop> inChunk = inWorld.computeIfAbsent(shopChunk,
                k -> new MapMaker().initialCapacity(1).makeMap());
    ...
    public void clear() {
        ...
        CompletableFuture<?> saveTask = CompletableFuture.allOf(plugin.getShopManager().getAllShops().stream()
                .filter(Shop::isDirty).map(Shop::update).toArray(CompletableFuture[]::new));
        try {
     ...
     */
    @Override
    public void createShop(@NotNull Shop shop, @Nullable Block signBlock, boolean bypassProtectionCheck)
            throws IllegalStateException
    {
        ...
        if (plugin.getShopItemBlackList().isBlacklisted(shop.getItem()) && !plugin.perm().hasPermission(p,
                "quickshop.bypass." + shop.getItem().getType().name().toLowerCase(Locale.ROOT)))
        {
        ...
        if (!bypassProtectionCheck) {
            ...
            if (!result.isSuccess()) {
                ...
                if (plugin.perm().hasPermission(p, "quickshop.alert")) {
                    ...
                    plugin.text()
                            .of(p, "3rd-plugin-build-check-failed-admin", result.getMessage(), result.getListener())
                            .send();
        ...
        PriceLimiterCheckResult priceCheckResult = this.priceLimiter.check(p, shop.getItem(), plugin.getCurrency(),
                shop.getPrice());
        switch (priceCheckResult.getStatus()) {

            case REACHED_PRICE_MIN_LIMIT -> plugin.text()
                    .of(p, "price-too-cheap",
                            Component.text((useDecFormat) ? MsgUtil.decimalFormat(priceCheckResult.getMax())
                                    : Double.toString(priceCheckResult.getMin())))
                    .send();
            case REACHED_PRICE_MAX_LIMIT -> plugin.text()
                    .of(p, "price-too-high",
                            Component.text((useDecFormat) ? MsgUtil.decimalFormat(priceCheckResult.getMax())
                                    : Double.toString(priceCheckResult.getMin())))
                    .send();
            case PRICE_RESTRICTED -> plugin.text()
                    .of(p, "restricted-prices", Util.getItemStackName(shop.getItem()),
                            Component.text(priceCheckResult.getMin()), Component.text(priceCheckResult.getMax()))
                    .send();
            ...
            case PASS -> {
                ...
                if (createCost > 0) {
                    ...
                    SimpleEconomyTransaction economyTransaction = SimpleEconomyTransaction.builder()
                            .taxAccount(cacheTaxAccount).taxModifier(0.0).core(plugin.getEconomy())
                            .from(p.getUniqueId()).to(null).amount(createCost).currency(plugin.getCurrency())
                            .world(shop.getLocation().getWorld()).build();
                    if (!economyTransaction.checkBalance()) {

                        plugin.text().of(p, "you-cant-afford-a-new-shop",
                                format(createCost, shop.getLocation().getWorld(), shop.getCurrency())).send();
                        return;
                    ...
                    if (!economyTransaction.failSafeCommit()) {
                        ...
                        plugin.logger().error("EconomyTransaction Failed, last error:{} ",
     ...
     * @deprecated Use getInteractiveManager instead. This way won't trigger the
     *             BungeeCord notification.
     */
     ...
     *
     * @param loc                  The location to get the shop from
     * @param skipShopableChecking whether to check is shopable, this will cause
     *                             chunk loading
     * @return The shop at that location
     */
    @Override
    ...
    public double getTax(@NotNull Shop shop, @NotNull QUser p) {
        ...
        if (shop.isUnlimited() && plugin.perm().hasPermission(p, "quickshop.tax.bypassunlimited")) {
            ...
            tax = 0;
            Log.debug("Disable the Tax for player " + p
        ...
        if (tax >= 1.0) {
            ...
            plugin.logger().warn("Disable tax due to is invalid, it should be in >=0.0 and <1.0 (current value is {})",
                    tax);
            tax = 0;
    ...
    public boolean isReachedLimit(@NotNull QUser p) {
        ...
        if (plugin.getRankLimiter().isLimit()) {
            ...
            try {
                ...
                additional = (int) Class.forName("dev.dan.quickshoplimit.QuickShopLimit")
                        .getMethod("getAdditionalCount", Player.class).invoke(additional, p.getBukkitPlayer().get());
    ...
    public CompletableFuture<?> unregisterShop(@NotNull Shop shop, boolean persist) {
        ...
        return plugin.getDatabaseHelper()
                .removeShopMap(loc.getWorld().getName(), loc.getBlockX(), loc.getBlockY(), loc.getBlockZ())
    ...
    private void refundShop(Shop shop) {
        ...
        if (plugin.getConfig().getBoolean("shop.refund")) {
            ...
            if (plugin.getConfig().getBoolean("shop.refund-from-tax-account", false)
                    && shop.getTaxAccountActual() != null)
            {

                cost = Math.min(cost,
                        plugin.getEconomy().getBalance(shop.getTaxAccountActual(), world, plugin.getCurrency()));
                transaction = SimpleEconomyTransaction.builder().amount(cost).core(plugin.getEconomy())
                        .currency(plugin.getCurrency()).world(world).from(shop.getTaxAccountActual())
                        .to(shop.getOwner()).build();

            } else {

                transaction = SimpleEconomyTransaction.builder().amount(cost).core(plugin.getEconomy())
                        .currency(plugin.getCurrency()).world(world).to(shop.getOwner()).build();
     ...
     */
    @Override
    public void sendPurchaseSuccess(@NotNull QUser purchaser, @NotNull Shop shop, int amount, double total,
            double tax)
    {
        ...
        if (showTax) {
            ...
            chatSheetPrinter.printLine(plugin.text()
                    .of(purchaser, "menu.item-name-and-price-tax", Component.text(amount * shop.getItem().getAmount()),
                            Util.getItemStackName(shop.getItem()), format(total, shop), tax)
                    .forLocale());

        } else {

            chatSheetPrinter.printLine(plugin.text()
                    .of(purchaser, "menu.item-name-and-price", Component.text(amount * shop.getItem().getAmount()),
                            Util.getItemStackName(shop.getItem()), format(total, shop))
                    .forLocale());
    ...
    public void sendSellSuccess(@NotNull QUser seller, @NotNull Shop shop, int amount, double total, double tax) {
        ...
        chatSheetPrinter.printLine(plugin.text().of(seller, "menu.item-name-and-price", amount,
                Util.getItemStackName(shop.getItem()), format(total, shop)).forLocale());
        if (showTax) {
            ...
            if (tax != 0) {
                ...
                if (!seller.equals(shop.getOwner())) {
                    ...
                    chatSheetPrinter
                            .printLine(plugin.text().of(seller, "menu.sell-tax", format(tax, shop)).forLocale());
    ...
    public void sendShopInfo(@NotNull Player p, @NotNull Shop shop) {
                ...
                || plugin.perm().hasPermission(p, "quickshop.other.preview"))
        {
            ...
            Component previewComponent = plugin.text()
                    .of(p, "menu.preview", Component.text(previewItemStack.getAmount())).forLocale()
                    .hoverEvent(plugin.getPlatform().getItemStackHoverEvent(previewItemStack))
                    .clickEvent(ClickEvent.clickEvent(ClickEvent.Action.RUN_COMMAND, MsgUtil
                            .fillArgs("/quickshop silentpreview {0}", shop.getRuntimeRandomUniqueId().toString())));
            ItemPreviewComponentPopulateEvent itemPreviewComponentPopulateEvent = new ItemPreviewComponentPopulateEvent(
                    previewComponent, p);
            itemPreviewComponentPopulateEvent.callEvent();
            previewComponent = itemPreviewComponentPopulateEvent.getComponent();
            chatSheetPrinter.printLine(plugin.text().of(p, "menu.item", Util.getItemStackName(shop.getItem()))
                    .forLocale().append(Component.text("   ")).append(previewComponent));
            ...
            previewComponentPrePopulateEvent.callEvent();
            chatSheetPrinter.printLine(plugin.text()
                    .of(p, "menu.item", Util.getItemStackName(previewComponentPrePopulateEvent.getItemStack()))
                    .forLocale().hoverEvent(plugin.getPlatform()
                            .getItemStackHoverEvent(previewComponentPrePopulateEvent.getItemStack())));
        ...
        if (Util.isTool(items.getType())) {
            ...
            chatSheetPrinter.printLine(plugin.text()
                    .of(p, "menu.damage-percent-remaining", Component.text(Util.getToolPercentage(items))).forLocale());
        ...
        if (shop.isSelling()) {
            ...
            if (shop.getRemainingStock() == -1) {
                ...
                chatSheetPrinter.printLine(plugin.text()
                        .of(p, "menu.stock", plugin.text().of(p, "signs.unlimited").forLocale()).forLocale());
            ...
            if (shop.getRemainingSpace() == -1) {
                ...
                chatSheetPrinter.printLine(plugin.text()
                        .of(p, "menu.space", plugin.text().of(p, "signs.unlimited").forLocale()).forLocale());
        ...
        if (shop.getItem().getAmount() == 1) {
            ...
            chatSheetPrinter.printLine(plugin.text()
                    .of(p, "menu.price-per", Util.getItemStackName(shop.getItem()), format(shop.getPrice(), shop))
                    .forLocale());

        } else {

            chatSheetPrinter
                    .printLine(plugin.text().of(p, "menu.price-per-stack", Util.getItemStackName(shop.getItem()),
                            format(shop.getPrice(), shop), shop.getItem().getAmount()).forLocale());
        ...
        if (items.getItemMeta() instanceof PotionMeta potionMeta) {
            ...
            if (potionEffectType != null) {
                ...
                try {
                    ...
                    plugin.logger().warn("Failed to handle translation for PotionEffect {}", potionEffectType.getKey(),
                            th);
            ...
            if (potionMeta.hasCustomEffects()) {
                ...
                for (PotionEffect potionEffect : potionMeta.getCustomEffects()) {
                    ...
                    try {
                        ...
                        plugin.logger().warn("Failed to handle translation for PotionEffect {}",
                                potionEffect.getType().getKey(), th);
    ...
    private void notifyBought(@NotNull QUser seller, @NotNull Shop shop, int amount, int stock, double tax,
            double total)
    {
        ...
        Util.asyncThreadRun(() -> {
            ...
            if (plugin.getConfig().getBoolean("show-tax")) {
                ...
                notify = plugin.text()
                        .of("player-bought-from-your-store-tax", seller, amount * shop.getItem().getAmount(),
                                Util.getItemStackName(shop.getItem()), this.formatter.format(total - tax, shop),
                ...
                notify = plugin.text()
                        .of("player-bought-from-your-store", seller, amount * shop.getItem().getAmount(),
                                Util.getItemStackName(shop.getItem()), this.formatter.format(total - tax, shop))
                        .forLocale(langCode).hoverEvent(plugin.getPlatform().getItemStackHoverEvent(shop.getItem()));
            ...
            if (stock == amount) {
                ...
                if (shop.getShopName() == null) {
                    ...
                    stockWarn = plugin.text()
                            .of("shop-out-of-stock", shop.getLocation().getBlockX(), shop.getLocation().getBlockY(),
                                    shop.getLocation().getBlockZ(), Util.getItemStackName(shop.getItem()))
                            .forLocale(langCode)
                            .hoverEvent(plugin.getPlatform().getItemStackHoverEvent(shop.getItem()));
                    ...
                    stockWarn = plugin.text()
                            .of("shop-out-of-stock-name", shop.getShopName(), Util.getItemStackName(shop.getItem()))
                            .forLocale(langCode)
                            .hoverEvent(plugin.getPlatform().getItemStackHoverEvent(shop.getItem()));
    ...
    private @NotNull BlockState processWaterLoggedSign(@NotNull Block container, @NotNull Block signBlock) {
        ...
        if (signBlockData instanceof WallSign wallSignBlockData) {
            ...
            plugin.logger().warn("Sign material {} not a WallSign, make sure you using correct sign material.",
    ...
    private int buyingShopAllCalc(@NotNull AbstractEconomy eco, @NotNull Shop shop, @NotNull Player p) {
        ...
        if (!shop.isUnlimited()) {
            ...
            // even if the shop is unlimited, the config option pay-unlimited-shop-owners is
            // set to
            // true,
        ...
        if (amount < 1) { // typed 'all' but the auto set amount is 0
            ...
            if (ownerCanAfford == 0 && (!shop.isUnlimited() || payUnlimitedShopOwner)) {
                ...
                // when typed 'all' but the shop owner doesn't have enough money to buy at least
                // 1
                ...
                plugin.text().of(p, "the-owner-cant-afford-to-buy-from-you",
                        plugin.getShopManager().format(shop.getPrice(), shop.getLocation().getWorld(),
                                shop.getCurrency()),
                        plugin.getShopManager().format(ownerBalance, shop.getLocation().getWorld(), shop.getCurrency()))
                        .send();
                return 0;
            ...
            plugin.text().of(p, "you-dont-have-that-many-items", Component.text(amount),
                    Util.getItemStackName(shop.getItem())).send();
            return 0;
        ...
        if (shop == null) {
            ...
            if (!fromAttach) {
                ...
                if (Util.isWallSign(currentBlock.getType())) {
                    ...
                    @Nullable
                    final Block half = Util.getSecondHalf(currentBlock);
                    if (half != null) {
        ...
        if (shop.isBuying()) {
            ...
            if (StringUtils.isNumeric(message)) {
                ...
                if (message.equalsIgnoreCase(tradeAllKeyword)) {
                    ...
                    // instead of output cancelled message (when typed neither integer or 'all'),
                    // just let
            ...
            if (StringUtils.isNumeric(message)) {
                ...
                if (message.equalsIgnoreCase(tradeAllKeyword)) {
                    ...
                    // instead of output cancelled message, just let player know that there should
                    // be positive
                    // number or 'all'
                    plugin.text().of(p, "not-a-integer", message).send();
                    Log.debug("Receive the chat " + message + " and it format failed: " + message);
    ...
    public CompletableFuture<@NotNull List<Shop>> queryTaggedShops(@NotNull UUID tagger, @NotNull String tag) {
        ...
        return CompletableFuture.supplyAsync(
                () -> plugin.getDatabaseHelper().listShopsTaggedBy(tagger, tag).stream().map(this::getShop).toList(),
    ...
    @Override
    public CompletableFuture<@Nullable Integer> removeTag(@NotNull UUID tagger, @NotNull Shop shop,
            @NotNull String tag)
    {
    ...
    private int sellingShopAllCalc(@NotNull AbstractEconomy eco, @NotNull Shop shop, @NotNull Player p) {
        ...
        if (!shop.isUnlimited()) {
            ...
            // should check not having items but having empty slots, cause player is trying
            // to buy
            // items from the shop.
        ...
        if (amount < 1) { // typed 'all' but the auto set amount is 0
            ...
            if (!shop.isUnlimited() && shopHaveItems < 1) {
                ...
                plugin.text().of(p, "shop-stock-too-low", Component.text(shop.getRemainingStock()),
                        Util.getItemStackName(shop.getItem())).send();
                return 0;
                ...
                plugin.text().of(p, "you-cant-afford-to-buy",
                        plugin.getShopManager().format(price, shop.getLocation().getWorld(), shop.getCurrency()),
                        plugin.getShopManager().format(balance, shop.getLocation().getWorld(), shop.getCurrency()))
                        .send();
    ...
    static class TagParser {
        ...
        public ParseResult parseSingleTag(String tag) throws IllegalArgumentException {
            ...
            List<Shop> shops = singleCaching.computeIfAbsent(tag,
                    (t) -> shopManager.queryTaggedShops(tagger, t).join());

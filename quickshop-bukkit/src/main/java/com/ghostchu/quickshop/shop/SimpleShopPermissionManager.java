...
public class SimpleShopPermissionManager implements ShopPermissionManager, Reloadable {
    ...
    private void initDefaultConfiguration(@NotNull File file) {
        ...
        for (BuiltInShopPermissionGroup group : BuiltInShopPermissionGroup.values()) {
            ...
            yamlConfiguration.set(group.getNamespacedNode(),
                    group.getPermissions().stream().map(BuiltInShopPermission::getNamespacedNode).toList());
    ...
    private void loadConfiguration() {
        ...
        if (!yamlConfiguration.isSet(namespace + ".everyone") || !yamlConfiguration.isSet(namespace + ".staff")
                || !yamlConfiguration.isSet(namespace + ".blocked"))
        {
            ...
            try {
                ...
                Files.move(file.toPath(), file.toPath().resolveSibling(
                        file.getName() + ".corrupted." + UUID.randomUUID().toString().replace("-", "")));
                loadConfiguration();

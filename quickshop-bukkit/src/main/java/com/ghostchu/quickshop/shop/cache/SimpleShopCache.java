...
public class SimpleShopCache implements SubPasteItem, ShopCache {
    ...
    public SimpleShopCache(@NotNull QuickShop plugin,
            @NotNull Map<@NotNull ShopCacheNamespacedKey, @NotNull Pair<@NotNull Function<Location, Shop>, @Nullable Cache<Location, BoxedShop>>> initArgs)
    {

        this.plugin = plugin;
        this.plugin.getPasteManager().register(plugin.getJavaPlugin(), this);
        for (Map.Entry<ShopCacheNamespacedKey, Pair<Function<Location, Shop>, Cache<Location, BoxedShop>>> entry : initArgs
                .entrySet())
        {

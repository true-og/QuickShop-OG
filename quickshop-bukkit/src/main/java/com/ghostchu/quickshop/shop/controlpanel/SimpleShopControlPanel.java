...
public class SimpleShopControlPanel implements ShopControlPanel {
    ...
    public @NotNull List<Component> generate(@NotNull Player sender, @NotNull Shop shop) {
        ...
        if (!plugin.perm().hasPermission(sender, "quickshop.setowner")) {
            components.add(plugin.text()
                    .of(sender, "menu.owner", shop.ownerName(locale))
                    .forLocale());
        } else {
            Component text;
            if (plugin.getConfig().getBoolean("shop.show-owner-uuid-in-controlpanel-if-op") && shop.isUnlimited()) {
                text = plugin.text()
                        .of(
                                sender,
                                "controlpanel.setowner-uuid",
                                shop.ownerName(locale),
                                shop.getOwner().toString())
                        .forLocale();
            } else {
                text = plugin.text()
                        .of(sender, "controlpanel.setowner", shop.ownerName(locale))
                        .forLocale();
            }
            components.add(text.hoverEvent(HoverEvent.showText(plugin.text()
                            .of(sender, "controlpanel.setowner-hover")
                            .forLocale()))
        ...
        if (plugin.perm().hasPermission(sender, "quickshop.unlimited")) {
            Component text = plugin.text()
                    .of(sender, "controlpanel.unlimited", MsgUtil.bool2String(shop.isUnlimited()))
                    .forLocale();
            Component hoverText =
                    plugin.text().of(sender, "controlpanel.unlimited-hover").forLocale();
            String clickCommand = MsgUtil.fillArgs(
                    "/quickshop silentunlimited {0}",
                    shop.getRuntimeRandomUniqueId().toString());
            components.add(text.hoverEvent(HoverEvent.showText(hoverText))
        ...
        if (plugin.perm().hasPermission(sender, "quickshop.create.buy")
            ...
            if (shop.isSelling()) {
                Component text =
                        plugin.text().of(sender, "controlpanel.mode-selling").forLocale();
                Component hoverText = plugin.text()
                        .of(sender, "controlpanel.mode-selling-hover")
                        .forLocale();
                String clickCommand = MsgUtil.fillArgs(
                        "/quickshop silentbuy {0}",
                        shop.getRuntimeRandomUniqueId().toString());
                components.add(text.hoverEvent(HoverEvent.showText(hoverText))
                        .clickEvent(ClickEvent.clickEvent(ClickEvent.Action.RUN_COMMAND, clickCommand)));
            } else if (shop.isBuying()) {
                Component text =
                        plugin.text().of(sender, "controlpanel.mode-buying").forLocale();
                Component hoverText = plugin.text()
                        .of(sender, "controlpanel.mode-buying-hover")
                        .forLocale();
                String clickCommand = MsgUtil.fillArgs(
                        "/quickshop silentsell {0}",
                        shop.getRuntimeRandomUniqueId().toString());
                components.add(text.hoverEvent(HoverEvent.showText(hoverText))
                        .clickEvent(ClickEvent.clickEvent(ClickEvent.Action.RUN_COMMAND, clickCommand)));
            }
        ...
        if (plugin.perm().hasPermission(sender, "quickshop.other.price")
                || (plugin.perm().hasPermission(sender, "quickshop.create.changeprice")
            ...
            Component hoverText =
                    plugin.text().of(sender, "controlpanel.price-hover").forLocale();
        ...
        if (plugin.isAllowStack()) {
            if (plugin.perm().hasPermission(sender, "quickshop.other.amount")
                ...
                Component text = plugin.text()
                        .of(sender, "controlpanel.stack", shop.getItem().getAmount())
                        .forLocale();
                Component hoverText =
                        plugin.text().of(sender, "controlpanel.stack-hover").forLocale();
        ...
        if (!shop.isUnlimited()) {
            ...
            if (plugin.perm().hasPermission(sender, "quickshop.refill")) {
                Component text = plugin.text()
                        .of(sender, "controlpanel.refill", shop.getPrice())
                        .forLocale();
                Component hoverText =
                        plugin.text().of(sender, "controlpanel.refill-hover").forLocale();
            ...
            if (plugin.perm().hasPermission(sender, "quickshop.empty")) {
                Component text = plugin.text()
                        .of(sender, "controlpanel.empty", shop.getPrice())
                        .forLocale();
                Component hoverText =
                        plugin.text().of(sender, "controlpanel.empty-hover").forLocale();
                String clickCommand = MsgUtil.fillArgs(
                        "/quickshop silentempty {0}",
                        shop.getRuntimeRandomUniqueId().toString());
                components.add(text.hoverEvent(HoverEvent.showText(hoverText))
                        .clickEvent(ClickEvent.clickEvent(ClickEvent.Action.RUN_COMMAND, clickCommand)));
            }
        ...
        if ((plugin.perm().hasPermission(sender, "quickshop.other.toggledisplay")
            ...
            Component text = plugin.text()
                    .of(sender, "controlpanel.toggledisplay", MsgUtil.bool2String(!shop.isDisableDisplay()))
                    .forLocale();
            Component hoverText =
                    plugin.text().of(sender, "controlpanel.toggledisplay-hover").forLocale();
            String clickCommand = MsgUtil.fillArgs(
                    "/quickshop silenttoggledisplay {0}",
                    shop.getRuntimeRandomUniqueId().toString());
            components.add(text.hoverEvent(HoverEvent.showText(hoverText))
                    .clickEvent(ClickEvent.clickEvent(ClickEvent.Action.RUN_COMMAND, clickCommand)));
        }
        ...
        // Remove
        if (plugin.perm().hasPermission(sender, "quickshop.other.destroy")
                || shop.playerAuthorize(sender.getUniqueId(), BuiltInShopPermission.DELETE)) {
            Component text = plugin.text()
                    .of(sender, "controlpanel.remove", shop.getPrice())
                    .forLocale();
            Component hoverText =
                    plugin.text().of(sender, "controlpanel.remove-hover").forLocale();
            String clickCommand = MsgUtil.fillArgs(
                    "/quickshop silentremove {0}",
                    shop.getRuntimeRandomUniqueId().toString());
            components.add(text.hoverEvent(HoverEvent.showText(hoverText))
                    .clickEvent(ClickEvent.clickEvent(ClickEvent.Action.RUN_COMMAND, clickCommand)));
        }

    ...
    public @NotNull List<Component> generate(@NotNull Player sender, @NotNull Shop shop) {
        ...
        if (!plugin.perm().hasPermission(sender, "quickshop.setowner")) {
            ...
            if (plugin.getConfig().getBoolean("shop.show-owner-uuid-in-controlpanel-if-op") && shop.isUnlimited()) {
                ...
                text = plugin.text()
                        .of(sender, "controlpanel.setowner-uuid", shop.ownerName(locale), shop.getOwner().toString())
                        .forLocale();
            ...
            components.add(text
                    .hoverEvent(
                            HoverEvent.showText(plugin.text().of(sender, "controlpanel.setowner-hover").forLocale()))
        ...
        if (plugin.perm().hasPermission(sender, "quickshop.unlimited")) {
            ...
            Component text = plugin.text().of(sender, "controlpanel.unlimited", MsgUtil.bool2String(shop.isUnlimited()))
                    .forLocale();
            Component hoverText = plugin.text().of(sender, "controlpanel.unlimited-hover").forLocale();
            String clickCommand = MsgUtil.fillArgs("/quickshop silentunlimited {0}",
                        ...
                        || plugin.perm().hasPermission(sender, "quickshop.create.admin")))
        {
            ...
            if (shop.isSelling()) {
                ...
                String clickCommand = MsgUtil.fillArgs("/quickshop silentbuy {0}",
                        shop.getRuntimeRandomUniqueId().toString());
                components.add(text.hoverEvent(HoverEvent.showText(hoverText))
                        .clickEvent(ClickEvent.clickEvent(ClickEvent.Action.RUN_COMMAND, clickCommand)));
                ...
                String clickCommand = MsgUtil.fillArgs("/quickshop silentsell {0}",
                        shop.getRuntimeRandomUniqueId().toString());
                components.add(text.hoverEvent(HoverEvent.showText(hoverText))
                        .clickEvent(ClickEvent.clickEvent(ClickEvent.Action.RUN_COMMAND, clickCommand)));
                        ...
                        && shop.playerAuthorize(sender.getUniqueId(), BuiltInShopPermission.SET_PRICE)))
        {
            ...
            Component text = MsgUtil.fillArgs(plugin.text().of(sender, "controlpanel.price").forLocale(),
                    LegacyComponentSerializer.legacySection()
                            .deserialize((plugin.getConfig().getBoolean("use-decimal-format"))
        ...
        if (!shop.isUnlimited()) {
            ...
            if (plugin.perm().hasPermission(sender, "quickshop.empty")) {
                ...
                String clickCommand = MsgUtil.fillArgs("/quickshop silentempty {0}",
                        shop.getRuntimeRandomUniqueId().toString());
                components.add(text.hoverEvent(HoverEvent.showText(hoverText))
                        .clickEvent(ClickEvent.clickEvent(ClickEvent.Action.RUN_COMMAND, clickCommand)));
                ...
                && plugin.isDisplayEnabled())
        {
            ...
            String clickCommand = MsgUtil.fillArgs("/quickshop silenttoggledisplay {0}",
                    shop.getRuntimeRandomUniqueId().toString());
            components.add(text.hoverEvent(HoverEvent.showText(hoverText))
                    .clickEvent(ClickEvent.clickEvent(ClickEvent.Action.RUN_COMMAND, clickCommand)));
                ...
                || shop.playerAuthorize(sender.getUniqueId(), BuiltInShopPermission.DELETE))
        {
            ...
            String clickCommand = MsgUtil.fillArgs("/quickshop silentremove {0}",
                    shop.getRuntimeRandomUniqueId().toString());
            components.add(text.hoverEvent(HoverEvent.showText(hoverText))
                    .clickEvent(ClickEvent.clickEvent(ClickEvent.Action.RUN_COMMAND, clickCommand)));

...
public class v1_20_R2 implements VirtualDisplayPacketFactory {
    ...
    public @NotNull PacketContainer createFakeItemMetaPacket(int entityID, @NotNull ItemStack itemStack) {
        ...
        PacketContainer fakeItemMetaPacket = manager.getProtocolManager()
        ...
        if (plugin.getConfig().getBoolean("shop.display-item-use-name")) {
            ...
            wpw.setObject(2, WrappedDataWatcher.Registry.getChatComponentSerializer(true),
        ...
        for (WrappedWatchableObject wrappedWatchableObject : wrappedWatchableObjects) {
            ...
            wrappedDataValues.add(new WrappedDataValue(watchableObject.getIndex(), watchableObject.getSerializer(),
    ...
    public @NotNull PacketContainer createFakeItemVelocityPacket(int entityID) {
        ...
        PacketContainer fakeItemVelocityPacket = manager.getProtocolManager()
    ...
    public @NotNull PacketContainer createFakeItemDestroyPacket(int entityID) {
        ...
        PacketContainer fakeItemDestroyPacket = manager.getProtocolManager()
        ...
        // On 1.17.1 (may be 1.17.1+? it's enough, Mojang, stop the changes), we need
        // add the int list
        // Entity to remove
        try {
    ...
    public @NotNull PacketAdapter getChunkSendPacketAdapter() {
        ...
        return new PacketAdapter(plugin.getJavaPlugin(), ListenerPriority.HIGH, PacketType.Play.Server.MAP_CHUNK) {
            ...
            public void onPacketSending(@NotNull PacketEvent event) {
                ...
                manager.getChunksMapping().computeIfPresent(new SimpleShopChunk(player.getWorld().getName(), x, z),
                        (chunkLoc, targetList) ->
                        {
    ...
    public @NotNull PacketAdapter getChunkUnloadPacketAdapter() {
        ...
        return new PacketAdapter(plugin.getJavaPlugin(), ListenerPriority.HIGH, PacketType.Play.Server.UNLOAD_CHUNK) {
            ...
            public void onPacketSending(@NotNull PacketEvent event) {
                ...
                StructureModifier<ChunkCoordIntPair> intPairStructureModifier = event.getPacket()

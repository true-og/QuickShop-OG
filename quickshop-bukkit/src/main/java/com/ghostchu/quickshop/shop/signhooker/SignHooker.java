...
public class SignHooker {
    ...
    public void registerListener() {
        chunkAdapter =
                new PacketAdapter(PLUGIN.getJavaPlugin(), ListenerPriority.HIGH, PacketType.Play.Server.MAP_CHUNK) {
                    @Override
                    public void onPacketSending(@NotNull PacketEvent event) {
                        ...
                        Map<Location, Shop> shops = PLUGIN.getShopManager()
                                .getShops(player.getWorld().getName(), x, z);
                        if (shops == null) {
                            return;
                        }
                        Bukkit.getScheduler()
    ...
    public void updatePerPlayerShopSign(Player player, Location location, Shop shop) {
        ...
        for (Sign sign : shop.getSigns()) {
            PLUGIN.getPlatform()
                    .sendSignTextChange(player, sign, PLUGIN.getConfig().getBoolean("shop.sign-glowing"), lines);
        }
    }
    ...
    public void updatePerPlayerShopSignBroadcast(Location location, Shop shop) {
        ...
        Collection<Entity> nearbyPlayers = world.getNearbyEntities(
                shop.getLocation(),

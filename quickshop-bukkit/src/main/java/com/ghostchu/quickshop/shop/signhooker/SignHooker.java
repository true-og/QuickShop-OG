...
public class SignHooker {
    ...
    public void registerListener() {
        ...
        chunkAdapter = new PacketAdapter(PLUGIN.getJavaPlugin(), ListenerPriority.HIGH,
                PacketType.Play.Server.MAP_CHUNK)
        {
            ...
            public void onPacketSending(@NotNull PacketEvent event) {
                ...
                Bukkit.getScheduler().runTaskLater(PLUGIN.getJavaPlugin(),
                        () -> shops.forEach((loc, shop) -> updatePerPlayerShopSign(player, loc, shop)), 2);
    ...
    public void updatePerPlayerShopSign(Player player, Location location, Shop shop) {
        ...
        Log.debug("Updating per-player packet sign: Player=" + player.getName() + ", Location=" + location + ", Shop="
                + shop.getShopId());
        List<Component> lines = shop.getSignText(PLUGIN.getTextManager().findRelativeLanguages(player));
        for (Sign sign : shop.getSigns()) {

            PLUGIN.getPlatform().sendSignTextChange(player, sign, PLUGIN.getConfig().getBoolean("shop.sign-glowing"),
                    lines);
    ...
    public void updatePerPlayerShopSignBroadcast(Location location, Shop shop) {
        ...
        Collection<Entity> nearbyPlayers = world.getNearbyEntities(shop.getLocation(), Bukkit.getViewDistance() * 16,
                shop.getLocation().getWorld().getMaxHeight(), Bukkit.getViewDistance() * 16);

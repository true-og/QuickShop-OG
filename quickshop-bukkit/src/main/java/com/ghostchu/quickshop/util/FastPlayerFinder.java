...
public class FastPlayerFinder implements PlayerFinder, SubPasteItem {
    ...
    private final Cache<UUID, Optional<String>> nameCache = CacheBuilder.newBuilder()
            .expireAfterAccess(3, TimeUnit.DAYS).maximumSize(5000).recordStats().build();
    ...
    public void bakeCaches() {
        ...
        try (FileReader reader = new FileReader(file)) {
            ...
            List<UserCacheBean> userCacheBeans = JsonUtil.getGson().fromJson(reader,
                    new TypeToken<List<UserCacheBean>>()
                    {
                    }.getType());
            List<UserCacheBean> fullCacheBeans = userCacheBeans.stream().filter(b -> b.getUuid() != null)
                    .filter(b -> b.getName() != null).toList();
    ...
    @Override
    public @Nullable String uuid2Name(@NotNull UUID uuid, boolean writeCache,
    ...
    @Override
    public @Nullable UUID name2Uuid(@NotNull String name, boolean writeCache,
            @NotNull ExecutorService executorService)
    {
    ...
    private Map<Object, CompletableFuture<?>> getExecutorRef(@NotNull ExecutorService executorService) {
        ...
        for (Map.Entry<WeakReference<ExecutorService>, Map<Object, CompletableFuture<?>>> entry : this.handling
                .entrySet())
        {
    ...
    @Override
    public @NotNull CompletableFuture<String> uuid2NameFuture(@NotNull UUID uuid, boolean writeCache,
            @NotNull ExecutorService executorService)
    {
        ...
        CompletableFuture<String> inProgress = (CompletableFuture<String>) handling.get(uuid);
        if (inProgress != null) {

            Log.debug("Reused " + inProgress + " for uuid2Name lookup: uuid=" + uuid + ", writeCache=" + writeCache
        ...
        CompletableFuture<String> future = CompletableFuture
                .supplyAsync(() -> resolver.uuid2Name(uuid, executorService, (name) ->
                {
    ...
    @Override
    public @NotNull CompletableFuture<UUID> name2UuidFuture(@NotNull String name, boolean writeCache,
            @NotNull ExecutorService executorService)
    {
        ...
        Map<Object, CompletableFuture<?>> handling = getExecutorRef(executorService);
        @SuppressWarnings("unchecked")
        CompletableFuture<UUID> inProgress = (CompletableFuture<UUID>) handling.get(name);
        if (inProgress != null) {

            Log.debug("Reused " + inProgress + " for name2Uuid lookup: name=" + name + ", writeCache=" + writeCache
                    + ", executorService=" + executorService);
            return inProgress;

        }

        CompletableFuture<UUID> future = CompletableFuture
                .supplyAsync(() -> resolver.name2Uuid(name, executorService, (uuid) ->
                {
    ...
    public static class PlayerFinderResolver {
        ...
        @Nullable
        public String uuid2Name(@NotNull UUID uuid, @NotNull ExecutorService executorService,
                @NotNull Consumer<String> endCallback)
        {

            String name = null;
            try (PerfMonitor perf = new PerfMonitor("Username Lookup - " + uuid)) {

                GrabConcurrentTask<String> grabConcurrentTask = new GrabConcurrentTask<>(executorService,
                        new DatabaseFindNameTask(plugin.getDatabaseHelper(), uuid), new BukkitFindNameTask(uuid),
                        new EssentialsXFindNameTask(uuid), new PlayerDBFindNameTask(uuid));
                name = grabConcurrentTask.invokeAll("Username Lookup - " + uuid, 10, TimeUnit.SECONDS,
                        Objects::nonNull);
                return name;
        ...
        @NotNull
        public UUID name2Uuid(@NotNull String name, @NotNull ExecutorService executorService,
                @NotNull Consumer<UUID> endCallback)
        {

            UUID uuid = UUID.nameUUIDFromBytes(("OfflinePlayer:" + name).getBytes(StandardCharsets.UTF_8));
            try (PerfMonitor perf = new PerfMonitor("UniqueID Lookup - " + name)) {

                GrabConcurrentTask<UUID> grabConcurrentTask = new GrabConcurrentTask<>(executorService,
                        new DatabaseFindUUIDTask(plugin.getDatabaseHelper(), name), new BukkitFindUUIDTask(name),
                        new EssentialsXFindUUIDTask(name), new PlayerDBFindUUIDTask(name));
                // This cannot fail.
                UUID lookupResult = grabConcurrentTask.invokeAll("UniqueID Lookup - " + name, 15, TimeUnit.SECONDS,
                        Objects::nonNull);
    ...
    static class DatabaseFindNameTask implements Supplier<String> {
        ...
        public String get() {
            ...
            try {
                ...
                Log.debug("Error: a exception created while query the database for username looking up: "
                        + e.getMessage());
                return null;
    ...
    static class DatabaseFindUUIDTask implements Supplier<UUID> {
        ...
        public UUID get() {
            ...
            try {
                ...
                Log.debug("Error: a exception created while query the database for unique id looking up: "
                        + e.getMessage());
                return null;

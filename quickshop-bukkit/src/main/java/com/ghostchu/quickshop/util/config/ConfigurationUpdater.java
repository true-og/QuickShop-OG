...
public class ConfigurationUpdater {
    ...
    private void brokenConfigurationFix() {
        ...
        try (InputStreamReader buildInConfigReader = new InputStreamReader(
                new BufferedInputStream(Objects.requireNonNull(plugin.getJavaPlugin().getResource("config.yml"))),
                StandardCharsets.UTF_8))
        {

            if (new ConfigurationFixer(plugin, new File(plugin.getDataFolder(), "config.yml"), plugin.getConfig(),
                    YamlConfiguration.loadConfiguration(buildInConfigReader)).fix())
            {
    ...
    public void update(@NotNull Object configUpdateScript) {
        ...
        for (Method method : getUpdateScripts(configUpdateScript)) {
            ...
            try {
                ...
                plugin.logger().info(
                        "[ConfigUpdater] Updating configuration from " + current + " to " + updateScript.version());
                ...
                try {
                    ...
                    plugin.logger().warn(
                            "Failed to execute update script {} for version {}: {}, plugin may not working properly!",
                            method.getName(), updateScript.version(), e.getMessage(), e);
                ...
                plugin.logger().warn(
                        "Failed execute update script {} for updating to version {}, some configuration options may missing or outdated",
                        method.getName(), method.getAnnotation(UpdateScript.class).version(), throwable);
    ...
    private void cleanupOldConfigs() throws IOException {
        ...
        try {
            ...
            if (new File(plugin.getDataFolder(), "messages.yml").exists()) {
                ...
                Files.move(new File(plugin.getDataFolder(), "messages.yml").toPath(),
                        new File(plugin.getDataFolder(), "messages.yml.outdated").toPath());

...
public class ConfigurationUpdater {
    ...
    private void brokenConfigurationFix() {
        try (InputStreamReader buildInConfigReader = new InputStreamReader(
                new BufferedInputStream(
                        Objects.requireNonNull(plugin.getJavaPlugin().getResource("config.yml"))),
                StandardCharsets.UTF_8)) {
            if (new ConfigurationFixer(
                            plugin,
                            new File(plugin.getDataFolder(), "config.yml"),
                            plugin.getConfig(),
                            YamlConfiguration.loadConfiguration(buildInConfigReader))
                    .fix()) {
    ...
    public void update(@NotNull Object configUpdateScript) {
        ...
        for (Method method : getUpdateScripts(configUpdateScript)) {
            try {
                ...
                plugin.logger()
                        .info("[ConfigUpdater] Updating configuration from " + current + " to "
                ...
                plugin.logger()
                        .warn(
                                ...
                                method.getName(),
                                method.getAnnotation(UpdateScript.class).version(),
                                throwable);
            }
        }
    ...
    public List<Method> getUpdateScripts(@NotNull Object configUpdateScript) {
        ...
        methods.sort(
                Comparator.comparingInt(o -> o.getAnnotation(UpdateScript.class).version()));
        return methods;
    }
    ...
    private void cleanupOldConfigs() throws IOException {
        ...
        try {
            if (new File(plugin.getDataFolder(), "messages.yml").exists()) {
                Files.move(
                        new File(plugin.getDataFolder(), "messages.yml").toPath(),

...
public class BuiltInCollects { // Statistic
    ...
    @MetricCollectEntry(
            dataType = MetricDataType.RESEARCH,
            moduleName = "Research - Addons or Compacts Discovered",
            description =
                    "QuickShop collects the QuickShop's addons/compacts (including 3rd-party) list that installed on your server to discover new addons/compacts so we can contact authors when we have major API changes, or use for improve exists official addons/compacts who have most of users using.")
    public CustomChart researchAddonsCompacts() {
        return new AdvancedPie("research_addons_or_compacts_discovered", () -> {
            ...
            for (Plugin discoverPlugin : Bukkit.getPluginManager().getPlugins()) {
                ...
                if (descriptionFile.getDepend().contains(myName)
                        || descriptionFile.getSoftDepend().contains(myName)) {
    ...
    @MetricCollectEntry(
            dataType = MetricDataType.RESEARCH,
            moduleName = "Research - Item Stacking Shop",
            description =
                    "We collect this for determine if we should push Item Stacking Shop to a feature that default enabled.")
    ...
    @MetricCollectEntry(
            dataType = MetricDataType.RESEARCH,
            moduleName = "Research - Protection Listener Blacklist",
            description =
                    "We collect this for determine if we should add common listener blacklist entry to default configuration.")
    ...
    @MetricCollectEntry(
            dataType = MetricDataType.RESEARCH,
            moduleName = "Research - Command Alias",
            description = "We collect this for determine if we should add/remove alias to default configuration.")
    ...
    @MetricCollectEntry(
            dataType = MetricDataType.STATISTIC,
            moduleName = "Statistic - Database Types",
            description = "We collect this so we can know the percent of different database types users.")
    public CustomChart statisticDatabaseTypes() {
        return new SimplePie(
                "statistic_database_types", () -> plugin.getDatabaseDriverType().name());
    }

    @MetricCollectEntry(
            dataType = MetricDataType.STATISTIC,
            moduleName = "Statistic - Database Product",
            description =
                    "We collect this so we can know the database vendor that users using, and provide dedicated driver if possible.")
    public CustomChart statisticDatabaseProject() {
        return new SimplePie("statistic_database_product", () -> {
            try (Connection connection = plugin.getSqlManager().getConnection()) {
                ...
                Log.debug("Populate statistic database version failed: "
    ...
    @MetricCollectEntry(
            dataType = MetricDataType.STATISTIC,
            moduleName = "Statistic - Database Version",
            description =
                    "We collect this so we can know the database versions that users using, and provide/update dedicated driver or drop the support for too old database versions.")
    ...
    @MetricCollectEntry(
            dataType = MetricDataType.STATISTIC,
            moduleName = "Statistic - Economy Types",
            description = "We collect this so we can know the percent of different economy types users.")
    public CustomChart statisticEconomyTypes() {
        return new SimplePie(
                "statistic_economy_types", () -> plugin.getEconomy().getName());
    }

    @MetricCollectEntry(
            dataType = MetricDataType.STATISTIC,
            moduleName = "Statistic - ItemMatcher",
            description = "We collect this so we can know the item matcher that users using, and improve it.")
    public CustomChart statisticItemMatcher() {
        return new SimplePie(
                "statistic_item_matcher", () -> plugin.getItemMatcher().getName());
    }

    @MetricCollectEntry(
            dataType = MetricDataType.STATISTIC,
            moduleName = "Statistic - DisplayImpl",
            description = "We collect this so we can know the which one item display impl most using, and improve it.")
    public CustomChart statisticDisplayImpl() {
        return new SimplePie(
                "statistic_displayimpl", () -> AbstractDisplayItem.getNowUsing().name());
    }

    @MetricCollectEntry(
            dataType = MetricDataType.RESEARCH,
            moduleName = "Statistic - All shops hosting across all servers",
            description =
                    "How many shops we can power across all servers? This research will used for performance tweak for components like shop managing/looking up/caching size etc.")
    public CustomChart statisticAllShops() {
        return new SingleLineChart(
                "statistic_all_shops_hosting_across_all_servers",
                () -> plugin.getShopManager().getAllShops().size());
    }

    @MetricCollectEntry(
            dataType = MetricDataType.STATISTIC,
            moduleName = "Statistic - Background Debug Logger",
            description = "We collect this so we can know the which one item display impl most using, and improve it.")
    ...
    @MetricCollectEntry(
            dataType = MetricDataType.RESEARCH,
            moduleName = "Research - ProtocolLib Version",
            description =
    ...
    @MetricCollectEntry(
            dataType = MetricDataType.STATISTIC,
            moduleName = "Statistic - Server Software Build Version",
            description =

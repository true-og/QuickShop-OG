...
public class RollbarErrorReporter {
    ...
    private final List<Class<?>> ignoredException = Lists.newArrayList(IOException.class, OutOfMemoryError.class,
            ProtocolException.class, InvalidPluginException.class, UnsupportedClassVersionError.class,
            LinkageError.class, SQLException.class);
    ...
    public RollbarErrorReporter(@NotNull QuickShop plugin) {
        ...
        Config config = ConfigBuilder.withAccessToken("feae3ac28ad84326b17c1c61e00bd981")
                .environment(Util.isDevEdition() ? "development" : "production").platform(Bukkit.getVersion())
                .codeVersion(QuickShop.getInstance().getVersion()).handleUncaughtErrors(false).build();
        ...
        plugin.getJavaPlugin().getLogger().setFilter(quickShopExceptionFilter); // Redirect log request passthrough our
                                                                                // error catcher.
    ...
    private void sendError0(@NotNull Throwable throwable, @NotNull String... context) {
        ...
        plugin.getPrivacyController().privacyReview(MetricDataType.DIAGNOSTIC, "RollbarErrorReporter",
                ...
                () ->
                {
                    ...
                    try {
                        ...
                        Arrays.stream(finalThrowable.getStackTrace()).forEach(
                                a -> Log.debug(a.getClassName() + "." + a.getMethodName() + ":" + a.getLineNumber()));
                        if (Util.isDevMode()) {
    ...
    public boolean canReport(@NotNull Throwable throwable) {
        ...
        if (!GameVersion.get(plugin.getPlatform().getMinecraftVersion()).isCoreSupports()) { // Ignore errors if user
                                                                                             // install quickshop on
                                                                                             // unsupported

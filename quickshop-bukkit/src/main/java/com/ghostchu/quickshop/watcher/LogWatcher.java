...
public class LogWatcher extends BukkitRunnable implements AutoCloseable {
    ...
    private static final DateTimeFormatter LOG_FILE_FORMATTER = DateTimeFormatter.ofPattern("yyyy-MM-dd")
            .withZone(ZoneId.systemDefault());
            ...
            if (!log.exists()) {
                ...
                if ((log.length() / 1024f / 1024f) > plugin.getConfig().getDouble("logging.file-size")) {
                    ...
                    do {
                        ...
                        targetPath = logPath
                                .resolve(ZonedDateTime.now().format(LOG_FILE_FORMATTER) + "-" + i + ".log.gz");
                        i++;

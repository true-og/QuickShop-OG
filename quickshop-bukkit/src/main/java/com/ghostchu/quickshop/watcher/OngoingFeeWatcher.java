...
public class OngoingFeeWatcher extends BukkitRunnable {
            ...
            if (!shop.isUnlimited() || !ignoreUnlimited) {
                        ...
                        plugin.getCurrency()) >= cost)
                {
                    ...
                    Util.mainThreadRun(() -> {
                        ...
                        SimpleEconomyTransaction transaction = SimpleEconomyTransaction.builder().allowLoan(allowLoan)
                                .currency(plugin.getCurrency()).core(plugin.getEconomy()).world(world).amount(finalCost)
                                .to(finalTaxAccount).from(shopOwner).build();
    ...
    public void removeShop(@NotNull Shop shop) {
        ...
        MsgUtil.send(shop, shop.getOwner(),
                plugin.text()
                        .of("shop-removed-cause-ongoing-fee", LegacyComponentSerializer.legacySection()
                                .deserialize("World:" + Objects.requireNonNull(shop.getLocation().getWorld()).getName()
                                        + " X:" + shop.getLocation().getBlockX() + " Y:"
                                        + shop.getLocation().getBlockY() + " Z:" + shop.getLocation().getBlockZ()))
                        .forLocale());

...
public class ShopDataSaveWatcher extends BukkitRunnable {
    ...
    public void run() {
        ...
        CompletableFuture<?>[] shopsToSaveFuture = plugin.getShopManager().getAllShops().stream()
                .filter(Shop::isDirty)
                .map(Shop::update)
        ...
        saveTask = CompletableFuture.allOf(shopsToSaveFuture)
                ...
                .exceptionally(e -> {
                    plugin.logger()
                            .warn("Error while saving shops, all failed shops will attempt save again in next time", e);
                    return null;
                });
    }
}

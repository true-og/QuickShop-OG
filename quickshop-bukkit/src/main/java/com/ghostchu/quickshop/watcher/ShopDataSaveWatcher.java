...
public class ShopDataSaveWatcher extends BukkitRunnable {
    ...
    public void run() {
        ...
        CompletableFuture<?>[] shopsToSaveFuture = plugin.getShopManager().getAllShops().stream().filter(Shop::isDirty)
                .map(Shop::update).toArray(CompletableFuture[]::new);
        saveTask = CompletableFuture.allOf(shopsToSaveFuture).thenAcceptAsync((v) -> {
        ...
        }, QuickExecutor.getShopSaveExecutor()).exceptionally(e -> {
